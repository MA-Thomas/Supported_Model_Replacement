#!/bin/bash
# Reduce PDAC-only results, then compose them with the audited base COVID results.
...
set -euo pipefail
: "${IRIS_RR_RUN_ENV:?Submit through submit_pdac_revision_round_robin_slurm.sh}"
