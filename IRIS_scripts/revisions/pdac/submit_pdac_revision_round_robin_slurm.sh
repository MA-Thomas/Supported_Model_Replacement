#!/bin/bash
# Submit the PDAC-only context-revision tournament without touching the base run.

set -euo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPOSITORY_ROOT=$(git -C "${SCRIPT_DIR}" rev-parse --show-toplevel)
HELPER="${SCRIPT_DIR}/../../shared/slurm_round_robin_helper.py"
WORKER="${SCRIPT_DIR}/../../tournament/run_directed_round_robin_array.slurm"
FINALIZER="${SCRIPT_DIR}/finalize_pdac_revision_round_robin_slurm.sh"

MODE=
REVISION_BUNDLE_ROOT=
BASE_RUN_ROOT=
RUN_ROOT=
REVISION_ID=pdac_no_splen_evac_grid4
REPLACE_EVALUATION=pdac
...
usage() {
    cat <<'EOF'
Usage:
  bash submit_pdac_revision_round_robin_slurm.sh \
    --mode pilot|full \
    --revision-bundle-root /path/to/pdac_no_splen_evac_grid4 \
    --base-run-root /path/to/completed/original/run \
    --run-root /path/to/new/revision/run [options]

The revision bundle root must contain pr/ and roc/ PDAC-only bundles. The base
run is read-only and must contain the completed original bundles, full plans,
and audited reductions. Full mode composes its unchanged COVID verdicts with
the new PDAC verdicts after both compact reductions pass audit.

Options:
  --revision-id ID                Default pdac_no_splen_evac_grid4
...
for METRIC in pr roc; do
    ...
    python3 "${HELPER}" check-revision-bundle --bundle "${BUNDLE}" \
        ...
        --expected-systems 60 --expected-endpoints 112 --expected-positive 19
...
echo "Prepared ${MODE} PDAC context revision"
...
ARRAY_COMMAND=(
    sbatch "${SBATCH_COMMON[@]}" --job-name "iris_pdac_rev_${MODE}"
...
FINAL_JOB_ID=$(sbatch "${SBATCH_COMMON[@]}" --job-name "iris_pdac_rev_${MODE}_finalize" \
