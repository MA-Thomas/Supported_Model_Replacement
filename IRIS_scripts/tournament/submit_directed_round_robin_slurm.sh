...
PROVIDER="${SCRIPT_DIR}/../providers/iris_score_provider.py"
...
Required scientific choice:
  ...
  Use a newly generated complete-F tournament configuration. The previous
  full_roster_selfgated_dual_selection_v1 configuration is deprecated and
  must not be used for a replacement run.
...
Examples:
  bash IRIS_scripts/tournament/submit_directed_round_robin_slurm.sh --mode pilot \
  ...
  bash IRIS_scripts/tournament/submit_directed_round_robin_slurm.sh --mode full \
    --config /path/to/complete_f_tournament_config.json \
    --run-root /data1/.../full_roster_round_robin \
...
if [ -z "${PREBUILT_BUNDLE_ROOT}" ] && [ ! -f "${PROVIDER}" ]; then
    echo "ERROR: missing score provider required to build bundles: ${PROVIDER}" >&2
    exit 1
fi
...
for METRIC in pr roc; do
    ...
    if [ -d "${BUNDLE}" ]; then
        ...
        python3 "${HELPER}" check-frozen-hybrid-bundle --bundle "${BUNDLE}"
