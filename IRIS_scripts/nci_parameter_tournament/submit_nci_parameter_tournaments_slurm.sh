#!/bin/bash
# Prepares ten NCI parameter tournaments and submits one combined shard array.
...
if [ -d "${PACKAGE_ROOT}" ]; then
    "${PIPELINE}" audit-prepared --package "${PACKAGE_ROOT}"
else
...
echo "Prepared 10 tournaments as ${TASK_ID} shards; array 0-${ARRAY_LAST}%${MAX_CONCURRENT}"
