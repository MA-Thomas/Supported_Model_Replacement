...
The separation is sharper than a matter of degree, because a bootstrap draws from the empirical distribution of one evaluation and can generate only recombinations of units already observed. Although that scheme may approximate the sampling distribution of a regular statistic well, it proceeds as though the observed sample exhausted the population capable of defeating the claim, which is the assumption an open test must avoid.

Table~\ref{tab:regimes} settles which mechanisms a replication may vary, but not how those replications were obtained. Recombination from one evaluation and separate observation are different sources of evidence for the same regime, so the two questions cut across each other: a multisite study resampled by site releases environmental variation and remains projected, while two independently collected test sets for one fixed model release very little and do not.
...
Error-statistical severity asks another question. It evaluates, through a sampling distribution, how readily a procedure would have exposed a specified error if that error were present \cite{mayo1996error}. Equation~\eqref{eq:support} returns no such probability. Its severity is structural: widening $\PSet$ and increasing $K$ add declared opportunities for defeat, while $\Regime$ states their kind.
...
Lower quantiles and expected shortfall remain defensible summaries of a replication distribution. They answer frequency or lower-tail questions. Equation~\eqref{eq:support} answers the narrower question posed here: what level is jointly retained by $K$ independent replications? Popper grounds the meaning of survival and corroboration; he does not select among the summaries that could express it.

\begin{remark}
The expected minimum can be posed directly, so nothing above depends on a representation theorem; the narrower objection is that it might be one arbitrary lower-tail choice among many. Suppose a summary $\rho$ of the law of $T\in[L,U]$ is law invariant, respects first-order stochastic dominance, returns $c$ for a constant effect, and is additive for comonotone effects. The Schmeidler representation then gives
\[
\rho(T)
=
L+\int_L^U g\{\Prb(T>s)\}\,ds
\]
for nondecreasing $g$ with $g(0)=0$ and $g(1)=1$, so that $g(u)$ is the weight carried by a level one replication retains with probability $u$ \cite{schmeidler1986integral,yaari1987dual}. Two independent challenges retain a level with probability $u_1u_2$, and requiring the weight of that conjunction to satisfy $g(u_1u_2)=g(u_1)g(u_2)$ admits only $g(u)=u^c$ for $c>0$; survival of exactly $K$ replications then fixes $c=K$ and returns Equation~\eqref{eq:survival-representation}. The argument identifies the family rather than the exponent, which stays a design commitment. Lower quantiles and expected shortfall fail the multiplicative condition, which is why they answer the frequency and tail-average questions instead of this one.
\end{remark}
...
Whether the scheme represents the declared regime and how well it estimates the target are separate questions, and the second is harder here than it would be for a mean. By Equation~\eqref{eq:quantile-weight} the replication distribution enters $S_K$ under the weight $K(1-v)^{K-1}$, which is uniform at $K=1$ and concentrates near the lower tail as $K$ grows. A bootstrap reproduces a lower tail less reliably than a centre, and least reliably when $n_+$ is small, since the worst replications available to it are bounded by recombinations of observed units. Raising $K$ therefore sharpens the claim while moving the estimate into the region where the resampling approximation is weakest. The prevalence infimum adds a second difficulty, because $\inf_\pi$ is directionally but not fully differentiable where the minimizer is not unique, which is a known route to bootstrap inconsistency rather than only to poor interval coverage. Increasing $B$ addresses neither, and simulation under designs resembling the intended application is the only check offered here.
...
\subsection{Intervals and grids}
...
A prospective fixed-model regime can change $n_+^{\mathrm{rep}}$ and $n_-^{\mathrm{rep}}$ to the planned evaluation counts while drawing from the observed class-conditional score pools.

\subsection{Observed replications}

The bootstrap is what one evaluation affords rather than what the criterion requires. Suppose instead that $R$ evaluations are collected independently under the declared regime: separate test sets for a fixed model, separate sites, or separate development runs each assessed on outcomes withheld from it. Each supplies one realized
\[
z_r
=
\inf_{\pi\in\PSet}\CNAP_\pi(D_r),
\]
and Equation~\eqref{eq:sorted-k} applies with $R$ in place of $B$. The estimand does not change, but the projection described in Section~\ref{sec:support} does, because the minima are then taken over replications that occurred. What limits the calculation is that $R$ is normally small, so $K$ must be well below $R$ and the U-statistic carries substantial variance. Where an evaluation supports both calculations, they answer one question with different warrants and belong in the same report.

\section{Problem IV: model replacement requires a paired, tie-neutral claim}
\label{sec:paired}
...
\subsection{The finite-design displacement}
...
Pairing removes variation shared by the two models, but the displacement of Section~\ref{sec:displacement} belongs partly to each score vector. Let model $A$ assign four distinct scores to four units and let model $B$ assign one common score. With two positive labels, two negative labels, and $\pi=1/2$, suppose the labels are uniformly assigned over the six possibilities. Neither score vector carries association with the outcomes.
...
\subsection{What the number says, and what it does not license}
...
Finally, a common normalized scale may appear to make results from separate studies directly comparable. It does not. A paired advantage requires joint predictions on shared units, which separate studies do not have. A leaderboard would need every entrant to face one common declared assessment and an estimator preserving a common finite-design anchor. Those requirements form a separate problem rather than a corollary of the present scale.
...
\midrule
Supported quantity &
Paired $\widehat\theta_{A:B}$ and the reversed orientation for replacement; raw $\SRobHat$ where one model is reported alone \\
...
Estimation &
Observed class counts, whether replications were resampled or independently observed, bootstrap count, prevalence grid or optimizer, and Monte Carlo errors \\
Uncertainty &
...
For a paired decision the raw tie-averaged supported advantage is the headline quantity, and no anchor is subtracted from it. A possible report is:
...
For paired replacement, both score vectors remain attached to the same sampled units in the outer and inner layers.
