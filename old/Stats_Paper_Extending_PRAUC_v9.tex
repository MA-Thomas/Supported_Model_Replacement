...
\right].
\]
The level must survive every declared prevalence and every replication, and widening $\PSet$ or raising $K$ gives the claim more opportunities for defeat. A stratified bootstrap estimates the quantity. For an isolated model, conditional permutation locates it against the chance level of its own finite design, and the calibrated ratio is what the report carries. For two models on the same units, the operator is applied to the lowest paired CNAP difference over $\PSet$, computed under a convention that averages over the orderings within each tie block. That convention makes the differential design anchor vanish identically wherever the outcome labels of one evaluation are equally likely in every arrangement, so the comparison keeps the raw scale and estimates no correction, at the cost of being conservative. A replacement claim requires the supported advantage to exceed a declared margin, with a proposed outer lower bootstrap bound above that margin as the inferential criterion. Where a benchmark regime is instead declared once for every entrant, a third reading becomes available: leaving the tie orderings to be drawn afresh in each replication makes the null law of the entire statistic independent of the score vector, so the anchor becomes a constant of the benchmark and models that never met on the same units can be placed on one chance-to-perfect scale. The three readings answer a single difficulty, the displacement of the finite-design chance level, in the three ways that the analyst's declarations permit.
\end{abstract}
...
Nothing requires the analyst to be the one who declares them. A field wanting to track many models can declare all three once and oblige every entrant to face the same challenge space. The declarations then stop being a property of an individual study and become a property of a shared test, and the values they produce acquire a comparability that no individual study can manufacture for itself. Section~\ref{sec:leaderboard} develops that case. The construction does not change; what changes is who does the declaring, and how much it buys.


The available predictions determine how the statement enters a decision. One model evaluated on held-out outcomes can be located relative to the chance level of its finite evaluation design. Compatible reports from separate studies then support separate design-relative interpretation, and not an ordering. Competing models evaluated on the same units permit a paired analysis of the advantage retained throughout $\PSet$. The paired quantity supplies performance evidence for a choice or replacement decision. Where a common benchmark regime has been declared, a third reading becomes available, and models that never met on the same units can be placed on one axis.

One difficulty runs through all three readings, and naming it here saves the reader from meeting it three times as a surprise. Chance normalization puts zero at population random ranking, but a finite evaluation design does not sit at zero, and where it does sit depends on the model's own score vector. The three readings answer this in three different ways. An isolated report estimates the displacement and divides by it. A paired comparison removes the \emph{difference} of two displacements by choosing a convention. A benchmark fixes one displacement for every entrant in advance. Which answer is available is settled by what the analyst is in a position to declare, and Sections~\ref{sec:calibration} to \ref{sec:leaderboard} take them in that order.
...
The anchor developed here is specific to one model by construction, and that is what makes the resulting value a design-relative statement rather than a position on a common scale. There is a second answer to the same difficulty, available only where the declarations of Section~\ref{sec:motivation} have been made once for a whole field, under which the anchor becomes a constant that no model influences. It requires machinery the next section introduces for another purpose, and Section~\ref{sec:leaderboard} returns to it. The two anchors are wanted for opposite reasons and neither retires the other.
...
Equation~\eqref{eq:jitter} locates the convention exactly, and it is worth pausing on where it places the expectation. Tie averaging integrates over the arbitrary orderings, and it does so before anything else in the construction happens. The alternative is to leave the ordering random and let the replication criterion see it. That is a different quantity answering a different question, and Section~\ref{sec:leaderboard} takes it up for reasons that have nothing to do with paired comparison; Proposition~\ref{prop:convention-order} says how the two are ordered.

The paper has now met this choice three times. Whether the prevalence infimum sits inside or outside the expectation, in Section~\ref{sec:infimum-inside}. Whether the comparison is formed before or after the marginal summaries, in Section~\ref{sec:marginal}. And now whether the tie ordering is averaged before or after the survival operator. In each case the inner placement gives the more demanding quantity, and in each case the outer placement is the one a reader computes without noticing that a choice was made.
...
The two readings developed so far both end in a number belonging to one study. Section~\ref{sec:calibration} locates a model against the chance level of its own design, and Section~\ref{sec:reporting} will insist that two such values are neither differenced nor ranked. Section~\ref{sec:paired} orders two models, but only where they met on the same units. Neither reading places twenty models from twenty studies on one axis, which is what a field trying to track progress wants.

This section develops the third reading. It asks for something the first two do not, namely that the declarations of Section~\ref{sec:motivation} be made once for everybody rather than separately by each analyst. What that buys is worth stating carefully, because the obvious guess about where the gain comes from is wrong in an instructive way.

\subsection{What a declared benchmark buys}
...
That is the whole of what makes the values comparable, and it is worth being blunt that no theorem was required to obtain it. Comparability comes from the shared test. It is not a property of the statistic, and it is not recovered by any clever rescaling; Section~\ref{sec:what-universal-means} explains why no rescaling should be able to recover it.

What the shared regime does not supply is a reading. Equation~\eqref{eq:leaderboard-raw} returns a level on the CNAP scale, and Section~\ref{sec:calibration} established that the level a finite design reaches on nothing at all is not zero. A board reporting raw supported levels is internally consistent and externally illegible: a reader cannot tell whether $0.31$ sits a third of the way from chance to perfect or a hair above noise. The remaining work is to say where chance sits, and the useful fact is that under one further choice it sits in the same place for everyone.

\subsection{Where the anchor goes}
...
The convention pays immediately.
...
This strengthens Proposition~\ref{prop:uniformity}, which asserted only that the means agree, and Appendix~\ref{app:tie-convention} now derives both statements from the one fact by choosing where to place the expectation over $\Rank$. Taking it first returns the tie-averaged convention and Proposition~\ref{prop:uniformity}. Leaving it in place returns Equation~\eqref{eq:common-anchor}. One property of strict rankings, two conventions, separated by nothing more than the order of two operations.
...
\end{equation}
Every entrant receives the same affine map, so $L_A>L_B$ exactly when $\SLead_A>\SLead_B$. The transformation therefore contributes legibility and not order. That is a real contribution, since a board is read by people who need to know what a number means, but it should not be mistaken for the thing that makes the board work.

The larger gain lies elsewhere and it is practical. The anchor of Section~\ref{sec:calibration} has to be recomputed for every model, and Section~\ref{sec:reporting} notes that it must be recomputed inside every outer resample as well, at a cost of a factor $M$ in permutations. Equation~\eqref{eq:common-anchor} is a constant of the benchmark. It is computed once, published with the regime, and never touched again, and the permutation layer disappears from the single-model report entirely. Appendix~\ref{app:anchor} gives the recipe and tabulated values.

\subsection{What the convention costs}
\label{sec:jitter-cost}

Nothing here is free, and the price falls on models that leave ties.

Under the jitter convention a tie block containing both outcomes contributes differently in different replications, and that variation is exactly what the support operator exists to charge for. The two conventions are consequently ordered, and the ordering is not an accident of any example.
...
Whether the charge is deserved is a question about what the board is for, and it does not have one answer. The case for it is that a model supplying no ordering within a block has supplied no ordering, that any deployment must nevertheless impose one, and that the resulting instability is a real consequence of information the model did not provide rather than an artifact of the scoring. The case against it is that a deployment selecting whole blocks never meets the instability, and for such a deployment the charge is fictitious. Both readings are defensible and they do not reconcile, which is why the tie convention sits among the declarations of Definition~\ref{def:benchmark-regime} rather than being settled here.

One consequence must be stated plainly, because a reader who misses it will think the paper contradicts itself. The board and the paired comparison of Section~\ref{sec:paired} use different conventions, so two models can rank one way on the board and the other way head to head. This is not an inconsistency. The board scores a deployed randomized ranking and charges for tie instability; the paired comparison scores ranking information and is neutral to arbitrary refinement by Proposition~\ref{prop:refinement}. They are different estimands, both are worth having, and a report giving one is not evidence about the other.

\subsection{What the estimator does to the anchor}
...
\subsection{How to read the headline values}
...
Where a field declares one regime for every entrant, the same operator supplies a common scale. Leaving tie orderings to be drawn afresh in each replication makes the null law of the whole statistic independent of the score vector, so the anchor becomes a constant of the benchmark, computed once and published with it. Comparability then comes from the shared test rather than from the calibration, which contributes legibility alone, and the practical gain is that the permutation layer disappears. The estimator does not inherit the invariance, and the resulting distortion raises a floor near chance without disturbing the ordering of informative models.
...
No score vector, no model, and no permutation enters at any point, which is the content of Proposition~\ref{prop:anchor-invariance} restated as a computation. The anchor should be tabulated and published with the benchmark specification, alongside $\PSet$ and $K$, so that entrants evaluate Equation~\eqref{eq:leaderboard-raw} on their own data and read Equation~\eqref{eq:leaderboard} off a fixed constant.
...
The spread contracts as the evaluation grows, at a rate between $n^{-1/2}$ and $n^{-2/3}$ over this range. That is slow. It is the same unhurried decay as the matched-pair artifact of Section~\ref{sec:paired-scope}, and it does not respond to severity: raising $K$ or widening $\PSet$ moves the numerator and the anchor together and leaves the displacement broadly where it was.

\subsection{Behavior among models carrying signal}

The question that decides whether the board is usable is not whether the anchor is exact but whether the distortion varies between the models being ranked. Table~\ref{tab:anchor-signal} addresses it directly. Positive scores are drawn from $N(\delta,1)$ and negative from $N(0,1)$, the latent score is then rounded to the stated number of levels per unit, and each granularity is measured against its \emph{own} population supported level obtained by direct simulation under $\RegimeL$. A distortion-free board requires the bias column to be constant down each block.
...
The reason is the one Section~\ref{sec:paired-anchor} gave when rejecting an additive correction for the paired anchor. A displacement obliged to vanish as separation approaches the ceiling, where $\CNAP=1$ leaves no room for it, cannot vary much among models that are close to the ceiling. What was an obstacle there is a reprieve here, and it is the same fact about the geometry of the scale in both places.

\subsection{Consequences and a candidate repair}

Three things follow for a benchmark.

The board should declare a floor. Section~\ref{sec:anchor-estimation} asks for it and Table~\ref{tab:anchor-decay} sets its scale: at $n=128$ the artifact reaches $0.016$, so a board of that size resolving differences of $0.01$ between entrants near zero is reporting tie structure. The floor belongs with the other declarations of Definition~\ref{def:benchmark-regime}, fixed before entries arrive.

Large benchmark evaluations are worth their cost twice over. They sharpen estimation in the ordinary way and they shrink the anchor artifact, and the second is a reason for size that does not appear in the single-model analysis of Section~\ref{sec:sparse}.
...
A benchmark entry is cheaper than either. Its anchor is a published constant, so an entry costs one bootstrap array and no permutation layer, and an outer resample costs one further array rather than a factor of $M$. The jitter convention needs no closed form at all, since the ordering is drawn rather than integrated, and Equation~\eqref{eq:tie-averaged-closed} is not required; a single uniform draw per tie block per replication suffices, which is why Section~\ref{sec:common-anchor} can adopt it without the computational apology that accompanies Equation~\eqref{eq:tie-averaged-ap}.
