...
% Null and best-case regimes, and the attainable ceiling.
\newcommand{\RegimeNull}{\Regime_{0}}
\newcommand{\RegimeMax}{\Regime^{\max}}
\newcommand{\RCAPmax}{\operatorname{R\text{-}CAP}^{\mathrm{sup,max}}_{\pi_\star}}
\newcommand{\RCAPmaxHat}{\widehat{\operatorname{R\text{-}CAP}}{}^{\,\mathrm{sup,max}}_{\pi_\star}}
...
\subsection{The null regime}
\label{sec:null-regime}

Chance is a property of a design, so it is declared the way the estimand is declared.

\begin{definition}[Null regime]
The null regime $\RegimeNull$ is $\Regime$ with the score-outcome association removed. It preserves the target population, the size and class-count design, the score distribution and its tie structure, the dependence structure, and the model-development stages. It imposes independence of score and outcome within the exchangeable unit of $\Regime$.
\end{definition}

The chance level is then the same functional evaluated under this second regime:
\begin{equation}
\RCAPnull
=
S_2(\Tpi;\RegimeNull).
\label{eq:null-estimand}
\end{equation}

Everything $\Regime$ specifies, $\RegimeNull$ specifies identically except the association. A regime that repeats model development carries that stage into $\RegimeNull$, so the fitted model is itself generated under the null. Section~\ref{sec:caveats} gives what this costs.

\subsection{Estimating the chance level}

Equation~\eqref{eq:null-estimand} is estimated by executing the entire reported pipeline on data from which the association has been removed, while holding everything else fixed. The null is a severe test in the sense of error-statistical inference~\cite{mayo1996error}. A model whose scores carry no information about the outcome would fail to exceed it, and the level it produces is the performance a no-signal regime supports under the same design.
...
\item For each permutation $m=1,\ldots,M$, run the complete estimation procedure of Section~\ref{sec:estimation}: generate $B$ replicates under the declared regime and apply the all-pairs estimator, yielding $\widehat{\operatorname{R\text{-}CAP}}{}^{\,\mathrm{sup},(m)}_{\pi_\star}$.
...
Three features follow. The null inherits the declared regime automatically, which is the same commitment the estimand already makes. Inference is distribution-free, requiring no asymptotic argument that sparse positive counts would violate. And $\RCAPnullHat$ is itself a diagnostic. Its sign reports which of the two distortions in Equation~\eqref{eq:two-distortions} dominates the design. A positive value means the upward bias of non-interpolated AP wins, which is the common case at small positive counts. A negative value means the disagreement penalty wins, and the repetitions disagree enough to push a no-signal design below the analytic zero. Either way the researcher reads the design's contribution directly instead of reconstructing it from $n_+$, $n_-$, and the tie pattern.

\subsection{The attainable ceiling}
\label{sec:ceiling}

Both endpoints of the scale are properties of the design.

\begin{proposition}[Exactness of the ceiling under separable scores]
...
The hypothesis of the proposition can fail for a reason that has nothing to do with the model's ranking. A score reported on a coarse scale forces observations of both classes into one threshold block. No assignment of outcomes to such a score vector separates the classes, so no model reporting on that scale reaches $\Tpi=1$. Take a five-level score with $n=200$, $n_+=20$, and $\pi_\star=0.10$. The best attainable chance-corrected effect is $0.44$. Normalizing against a ceiling of one charges the model for resolution its measurement scale never offered.

The ceiling is therefore measured the way the chance level is, by running the estimator on data that fixes the quantity of interest at its extreme. Let $D^{\max}$ assign the $n_+$ positive outcomes to the $n_+$ highest scores of $D_{\mathrm{obs}}$, and let $\RegimeMax$ denote $\Regime$ applied to that evaluation. The attainable ceiling is
\begin{equation}
\RCAPmax
=
S_2(\Tpi;\RegimeMax),
\label{eq:ceiling-estimand}
\end{equation}
estimated by $\RCAPmaxHat$, the same $B$-replicate procedure run on $D^{\max}$. That assignment is optimal because moving a positive outcome into a lower threshold block, and a negative outcome into a higher one, cannot increase $\AP_{\pi_\star}$.

\begin{remark}
The ceiling is constructed, not searched. Sampling the label-permutation orbit and taking its maximum badly understates it, since random permutations almost never approach the optimum. In the five-level design above, $4{,}000$ sampled permutations reach $0.13$ against a true value of $0.44$.
\end{remark}

When the $n_+$th and $(n_++1)$th largest scores differ, $D^{\max}$ separates the classes, the proposition applies, and $\RCAPmaxHat=1$. A tie spanning that boundary is the only way the ceiling falls below one. Continuous scores therefore recover the fixed endpoint, and one additional bootstrap run, against $M$ runs for the null, establishes it.

\subsection{The calibrated score}

Two measured anchors define the reported scale.

\begin{definition}[Calibrated replication-adjusted chance-corrected average precision]
\begin{equation}
\RCAPcal
=
\frac{S_2(\Tpi;\Regime)-S_2(\Tpi;\RegimeNull)}
     {S_2(\Tpi;\RegimeMax)-S_2(\Tpi;\RegimeNull)}
=
\frac{\RCAP-\RCAPnull}{\RCAPmax-\RCAPnull}.
\label{eq:calibrated-estimand}
\end{equation}
\end{definition}

R-CAP is the position of the supported effect between the chance level of its design and the best that design permits. Both reference points are functionals of declared regimes, so the headline quantity is a population estimand on the same footing as R-CAP\textsuperscript{sup}. The plug-in estimate is
\begin{equation}
\RCAPcalHat
=
\frac{\RCAPHat-\RCAPnullHat}{\RCAPmaxHat-\RCAPnullHat},
\label{eq:calibrated}
\end{equation}
which requires $\RCAPmaxHat>\RCAPnullHat$. Equation~\eqref{eq:calibrated} has the form of Equation~\eqref{eq:kappa-form} applied one level up, with the chance value and the ceiling both measured rather than derived. Under separable-capable scores $\RCAPmaxHat=1$ and it reduces to
\[
\RCAPcalHat
=
\frac{\RCAPHat-\RCAPnullHat}{1-\RCAPnullHat},
\]
which is the form to expect whenever scores are continuous.

The two applications of Equation~\eqref{eq:kappa-form} compose cleanly, for a reason worth recording.

\begin{proposition}[Affine rescaling commutes with the support operator]
For constants $a>0$ and $c$, $S_2(aT+c;\Regime)=aS_2(T;\Regime)+c$.
\end{proposition}

\begin{proof}
For $a>0$, $\min(at_1+c,at_2+c)=a\min(t_1,t_2)+c$. Take expectations under $\Regime$.
\end{proof}

Chance correction is such a rescaling, so
\[
S_2(\Tpi;\Regime)
=
\frac{S_2(\AP_{\pi_\star};\Regime)-\pi_\star}{1-\pi_\star}.
\]
Correcting each repetition and then taking support returns what taking support and then correcting returns. Equation~\eqref{eq:calibrated-estimand} is the same rescaling with measured constants, so it commutes in the same way. The two levels of Equation~\eqref{eq:kappa-form} compose into a single operation instead of accumulating into two.
...
Both anchors depend on the score multiset, and two models evaluated on the same units rarely share one. Their tie patterns differ, so each model carries its own $\RCAPnullHat$ and $\RCAPmaxHat$. Calibration is then a different transformation for each model and it can reorder them. That is the intended behavior. A model reaching $\Tpi=0.4$ where its design permits $0.5$ has used most of the resolution available to it, and a model reaching $0.4$ where its design permits $1$ has not. Ordering is preserved when the anchors coincide, which requires a shared score multiset. Across studies, where $n_+$, $n_-$, and the tie pattern all differ, both anchors drift and the uncalibrated scale is not comparable at all.

The ceiling of Equation~\eqref{eq:ceiling-estimand} is attained under the canonical fixed-model regime. Under regimes that repeat training or draw new sites, the best-case labelling need not survive resampling, and $\RCAPmaxHat$ then estimates what the regime permits rather than marking a fixed endpoint.
...
Calibration converts a single loop of $B$ replicates into $M$ additional loops, plus one further loop for the ceiling of Equation~\eqref{eq:ceiling-estimand}. The support functional remains $O(B\log B)$ by Appendix~\ref{app:computation}, but it is not the dominant term. Each replicate must be resampled and its average precision recomputed, at cost $O(n)$ for the threshold sweep, so the total is $O\{MB(n+\log B)\}$. For any realistic evaluation $n$ exceeds $\log B$, and the resampling sweep rather than the sorting sets the cost. The permutations are mutually independent and therefore embarrassingly parallel, which is what keeps the null affordable for a fixed model. Only retraining regimes make it genuinely expensive, and there the same constraint already applies to the estimate itself.
...
\RCAPcalHat .
\]
Each is the one above it after a single adjustment, and each adjustment is the gap between neighbors.
...
$\RCAPcalHat$ & position between measured chance and the attainable ceiling & calibration \\
\bottomrule
\end{tabularx}
\end{table}

$\RCAPcalHat$ is the headline. Both of its anchors are measured, so it is interpretable within one design and comparable across designs.

The mean $\widehat{\E}[\Theta_{\pi_\star}]$ is required alongside it because Equation~\eqref{eq:decomposition} subtracts, and a difference cannot report its operands. Section~\ref{sec:collision} gives two designs with opposite replication behavior and the same $\RCAPHat$. Reporting the mean separates them completely, since Equation~\eqref{eq:decomposition} is an identity and any two of its three quantities determine the third.

Report the anchors as well. The null level $\RCAPnullHat$, the ceiling $\RCAPmaxHat$, and the permutation $p$-value let a reader confirm that the estimate lies above its measured chance level, which is the range in which the calibrated scale is valid.
...
\end{axis}
\end{tikzpicture}
\caption{The replication distribution under the declared regime (solid) and under the permutation null (dashed). Each distribution is summarized by its mean and by the support functional, which sits below the mean by the disagreement penalty. The calibrated score in Equation~\eqref{eq:calibrated} measures the distance from $\RCAPnull$ to $\RCAP$ relative to the distance from $\RCAPnull$ to the attainable ceiling $\RCAPmax$, which equals one whenever no score tie spans the boundary between the $n_+$ highest scores and the rest. The null here lies above zero, which is the typical situation at small positive counts.}
...
\item $\RCAPcalHat$ as the headline value;
\item the quantities that fix its scale:
      $\widehat{\E}[\Theta_{\pi_\star}]$, $\RCAPHat$,
      $\RCAPnullHat$, and $\RCAPmaxHat$;
...
Section~\ref{sec:reporting} requires $\widehat{\E}[\Theta_{\pi_\star}]$ for this reason. Equation~\eqref{eq:decomposition} is an identity, so any two of its three quantities fix the third. Read as mean and penalty, the first design is $(0.500,0.200)$ and the second is $(0.300,0.000)$. The separation is exact rather than approximate.
...
A reader who wants the effect estimate and its uncertainty separately should read them separately; Section~\ref{sec:reporting} supplies the estimate and Section~\ref{sec:error} the uncertainty. The claim made for the supported score is not that it replaces an interval, but that it defines a single population functional with a stated replication meaning, which an interval does not.

\subsection{Monte Carlo error and estimation error}
\label{sec:error}

$\RCAPHatB$ misses $\RCAP$ for two reasons, and the two respond to different remedies.

Monte Carlo error comes from finite $B$. Equation~\eqref{eq:uestimator} is a U-statistic of degree two, so its standard error follows from the Hoeffding projection $h_1(t)=\E[\min(t,\Theta_{\pi_\star})]$~\cite{hoeffding1948ustat}. On the sorted replicate array of Appendix~\ref{app:computation},
\begin{equation}
h_1(t_{(i)})
=
\frac{1}{B}
...
=
\frac{2}{\sqrt{B}}
\operatorname{sd}\{h_1(t_{(1)}),\ldots,h_1(t_{(B)})\}.
\label{eq:mcse}
\end{equation}
The projection is one prefix sum over an array the estimator has already sorted, so the standard error costs nothing beyond the estimate. It falls at rate $B^{-1/2}$, and it is the quantity a Monte Carlo stability criterion should target.

Estimation error comes from $\PRepHat$ differing from $\PRep$. Increasing $B$ does not touch it. It is governed by $n_+$ and $n_-$, and Section~\ref{sec:sparse} identifies it as the binding constraint in sparse designs. A nested bootstrap measures it. Resample evaluation units to form outer datasets, run the complete estimator within each, and read the spread of the resulting values. The cost is the outer replicate count multiplied by the cost of one complete estimate. L-moment estimators are asymptotically normal with known influence functions~\cite{hosking1990lmoments}, which supplies the same quantity analytically once $n_+$ is large enough for the approximation to hold, which is the regime in which it is least needed.

Equation~\eqref{eq:mcse} is cheap enough to report in every application. A study whose conclusion turns on the distance between $\RCAPHat$ and its measured null should give the nested-bootstrap spread as well.
...
Analytic chance correction is not sufficient at finite sample sizes. Non-interpolated average precision is upward biased under random ranking, the minimum imposes a downward disagreement penalty, and the residual after cancellation depends on the design. Both endpoints of the scale are therefore measured under the same declared regime, the lower one by a permutation null and the upper one by the best labelling the observed score vector admits, and the calibrated score
\[
\RCAPcalHat
=
\frac{\RCAPHat-\RCAPnullHat}{\RCAPmaxHat-\RCAPnullHat}
\]
places the supported effect between them. The ceiling equals one whenever no score tie spans the boundary between the $n_+$ highest scores and the rest, which returns the familiar form for continuous scores. The calibrated effect is the primary reported value. The observed effect, expected repeated performance, and the supported effect are reported with it, so that each adjustment separating them is visible.

Simulation and empirical study remain necessary to establish finite-sample behavior, sensitivity to sparse positives, robustness to dependence and regime misspecification, the adequacy of the canonical bootstrap, and the stability of the permutation null under broad regimes. The proposal supplies a defined population target, an operational estimator for a common setting, and a measured origin for the resulting scale. Reporting the observed effect with expected repeated performance and its replication-supported and calibrated counterparts separates the performance seen in one dataset from the performance claim sustained by repetition of the declared evaluation design.
...
Equation~\eqref{eq:quantile} expresses $S_K$ as a distortion measure with distortion function $g(u)=1-(1-u)^{K}$, a member of the dual-power family in the sense of Wang~\cite{wang1996premium}. Because $g$ is concave, the following hold immediately: monotonicity with respect to first-order stochastic dominance; translation equivariance, $S_2(T+c)=S_2(T)+c$; positive homogeneity, $S_2(aT)=aS_2(T)$ for $a>0$; comonotone additivity; superadditivity on gains, $S_2(X+Y)\geq S_2(X)+S_2(Y)$, with equality exactly when $X$ and $Y$ are comonotone; and concavity as a functional of the gain variable. The monotonicity result proved directly below is the first of these, and Section~\ref{sec:paired} uses the superadditivity.

\paragraph{Why this operator.}
These properties are not a coincidence of the construction. A functional on replication distributions that is law invariant, monotone, positively homogeneous, and comonotone additive is a distortion measure, by the representation results of Schmeidler~\cite{schmeidler1986integral} and Yaari~\cite{yaari1987dual}. Requiring in addition that it return $\E[\min(T_1,\ldots,T_K)]$, which is the $K$-repetition reading of Section~\ref{sec:support}, fixes $g(u)=1-(1-u)^{K}$ within that class. Comonotone additivity is the substantive axiom. It says that repetitions which vary together supply no corroboration beyond their common movement, which is the corroboration criterion stated in distributional terms. The support operator is therefore forced by the axioms rather than selected from a menu.
...
The calibrated score of Section~\ref{sec:calibration} repeats this calculation once per permutation. Sorting contributes $O(MB\log B)$ in total. The $MB$ resampling steps that produce the replicate effects contribute $O(MBn)$, since each rebuilds the threshold counts and sweeps them once. The second term dominates whenever $n>\log B$, which covers every evaluation large enough to be worth reporting. The permutations are mutually independent and parallelize without communication, and $M$ between $200$ and $500$ suffices for the location estimate $\RCAPnullHat$. A small permutation $p$-value requires larger $M$, and regimes that repeat model development multiply the cost by the training time rather than by the sorting cost.
