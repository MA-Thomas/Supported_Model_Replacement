...
\begin{abstract}
Predictive performance supports two tasks: interpreting the evidence for a model considered on its own and informing a choice between models evaluated together. We develop a replication-based framework for both tasks using average precision. Prior standardization evaluates precision at a common reference prevalence $\pi_\star$, and chance normalization creates a CNAP scale with population random ranking at zero and perfect ranking at one. For a scalar performance effect $T$ and $K$ independent executions of a declared evaluation-generating regime $\Regime$, define
\[
S_K(T;\Regime)
=
\E_{\Regime}[\min\{T_1,\ldots,T_K\}].
\]
The supported level is the average maximal claim surviving all $K$ executions. At $K=2$, it equals expected performance minus half the expected absolute disagreement between replications. The regime declares the mechanisms and environments that can challenge the claim, while $K$ controls the number of independent opportunities for its defeat. For fixed predictions on held-out outcomes, a stratified bootstrap estimates supported performance, and conditional permutation locates one model relative to its finite-design chance level. Compatible reports can support cautious benchmarking when joint predictions are unavailable. When models are evaluated on the same units, supported superiority applies the operator to paired CNAP differences. Pairing preserves shared variation in evaluation difficulty, while differencing marginal supported values generally overstates the retained advantage. A candidate meets the performance-based replacement criterion when its supported advantage exceeds a declared margin $d$; population-level evidence requires an outer lower confidence bound above $d$. The Bank Marketing illustration shows how replication support and replacement conclusions can change despite similar observed performance.
\end{abstract}
...
\section{Comparability, corroboration, and model choice}
\label{sec:problems}
...
The common scale still describes one finite result. Consider two evaluations that each report $0.30$, one with 20 positive cases and the other with 20,000. Each positive changes recall by $0.05$ in the smaller evaluation and by $0.00005$ in the larger one. A few ranks can move the smaller evaluation much more. Equal point estimates can therefore have different behavior across replications because of evaluation size, clustering, ties, influential cases, sites, measurement conditions, or repeated model development.

Replication gives a performance claim opportunities to fail. A claim for one fitted model can face new evaluation units. A claim intended for new sites must also face site variation. A claim about a development procedure must survive new training, tuning, and model-selection runs. Supported performance records the level retained across independent executions of the regime chosen to represent these challenges.

The available predictions determine how this idea enters practice. When one model is available, conditional calibration locates its supported performance relative to the chance level of its finite evaluation design. Compatible reports from separate studies can then support cautious benchmarking. When competing models are available on the same units, the relevant quantity is their supported paired difference. It preserves shared changes in evaluation difficulty and addresses a performance-based choice or replacement decision.

Both uses require a target prevalence for scale comparability and a regime for corroboration. Studies sharing $\pi_\star$ answer the same corroboration question only under compatible regimes, including compatible replication sizes. The regime may hold the fitted model and evaluation environment fixed while redrawing units, allow contextual conditions to vary across replications, or repeat the complete development process.
...
\label{eq:odds}
\end{equation}
Precision odds are a product of the target-population prior odds and a threshold-specific likelihood ratio determined by the class-conditional score distributions. Equation~\eqref{eq:main-reference-precision} replaces the prior-odds factor with a declared value and leaves the ranking information unchanged. The same factorization underlies prior-probability and cost adjustment in classification~\cite{elkan2001foundations}.
...
The reference prevalence sets the value assigned to each recall increment. A smaller value weights false positives more heavily and emphasizes the leading part of the ranking. A larger value credits separation over a broader recall range. Appendix~\ref{app:reference-behavior} gives the limiting and monotonicity calculations.
...
The regime determines the sources of failure against which the claim is assessed. The functional $S_K(T;\Regime)$ may be well-defined when the available data do not identify it. An empirical replication procedure generates only the challenges represented by its data and assumptions. Table~\ref{tab:regimes} gives three common regimes.
...
Independence applies to complete replications and permits dependence among observations within a replication. Resampling and permutation operate on the exchangeable unit allowed by the design. Repeated-development regimes rerun the complete fitting pipeline. Appendix~\ref{app:regime-details} gives hierarchical notation and implementation details.
...
\section{The replication-survival support operator}
\label{sec:support}
...
\min\{t_1,t_2\}.
\]
The rule is symmetric, and equal results retain their common value. It expresses corroboration through survival under independent opportunities for defeat~\cite{popper1959logic}.
...
\end{equation}
When a replication distribution $P$ is supplied directly, $S_K(T;P)$ denotes the same expectation for independent draws from $P$.

Equation~\eqref{eq:support-operator} averages the greatest retained level over sets of $K$ replications generated by the regime. It remains on the scale of $T$. If $T$ is a chance-normalized performance effect, $S_K$ is expressed in the same units.

When $T$ is bounded below by $L$ and above by $U$,
\begin{equation}
S_K(T;\Regime)
=
L+\int_L^U \Pr_{\Regime}(T>s)^K\,ds .
\label{eq:joint-survival}
\end{equation}
At each effect level $s$, the powered survival probability is the probability that all $K$ replications retain the claim. The integral averages that joint survival over the effect scale. Appendix~\ref{app:joint-survival-proof} gives the derivation.

The regime controls the kinds of challenges a claim faces. The value of $K$ controls how many independent challenges it must survive. Increasing $K$ gives the claim more opportunities for defeat and lowers $S_K$ for any nondegenerate replication distribution. The default $K=2$ is the smallest value that tests joint survival.

At $K=2$, the identity $\min(a,b)=(a+b-|a-b|)/2$ gives
\begin{equation}
S_2(T;\Regime)
=
\E_{\Regime}[T]
...
The residual can be practically visible. Under random ranking with $n=10$, one positive case, and $\pi_\star=0.1$, the supported chance level is $0.0786$: a model with no information earns a positive raw supported score in that finite design. The sign and magnitude of $\beta-\delta$ depend on the class counts, ties, and $\pi_\star$. Conditional permutation measures their net contribution for the observed score vector. Appendix~\ref{app:null-examples} gives finite-design examples and the general one-positive formula.
...
Appendix~\ref{app:fixed-point-proof} gives the proof. The fixed point belongs to the canonical bootstrap. Under regimes that rerun training or draw new sites, observed separation need not survive replication, so the calibration developed here remains confined to fixed predictions.
...
The fixed upper anchor treats limited score resolution as a performance limitation and preserves a common scale across score representations. Appendix~\ref{app:calibration-details} discusses this choice. Affine equivariance allows chance normalization to be applied before or after the support operator; Appendix~\ref{app:affine-equivariance} records the identity.
...
The permutation distribution represents the sharp null $Y\perp S$ conditional on the realized scores and permitted outcome assignments. Validity requires that no evaluation outcome enter the mechanism producing those scores and that the exchangeable unit match the evaluation design. Held-out predictions satisfy the first requirement. Cross-fitted predictions and repeated-development regimes require null procedures that preserve their dependence on outcomes. Appendix~\ref{app:conditional-validity} treats these cases in detail, and Appendix~\ref{app:computation} gives the computational cost.
...
\]
Applying the support operator to this paired replication distribution defines supported superiority:
\begin{equation}
...
Appendix~\ref{app:marginal-proof} gives the proof and a numerical example. Differencing marginals therefore never understates model $A$'s supported advantage. It compares separate replication summaries. Supported superiority applies the criterion to within-replication differences and requires common paired replications. Marginal supported values from separate studies cannot establish it.
...
The disagreement penalty can skew the estimator, and sparse positive counts or heavy ties increase discreteness near zero. Appendix~\ref{app:superiority-intervals} discusses interval choices that require further simulation.
...
\subsection{Evaluation size and supported performance}
...
Appendix~\ref{app:properties} gives the operator's mathematical properties and limiting behavior.
...
$\SAPcalHat$ is the calibrated headline when the supported effect clears its conditional null. Report $\SAPnullHat$, the permutation $p$-value, and the replication mean with it. A complete report also gives $\pi_\star$, observed and replication counts, whether the regime is same-design or prospective, $K$, the resampling and permutation methods, exchangeable units, Monte Carlo stability, and the separation of prediction development from evaluation outcomes. Appendix Figure~\ref{fig:distribution} relates the replication mean, supported level, and conditional null, and Appendix~\ref{app:report-example} gives a complete specimen report.
...
Three fixed model specifications were fitted. A full logistic regression and a random forest used all available pre-contact features and provided the close replacement comparison. A demographic logistic regression used age, job, marital status, and education and supplied a larger positive-control contrast. Categorical variables were represented by one-hot indicators. Call duration was excluded because its value is known only after the contact whose outcome is being predicted. Every reported score came from the temporal holdout.
...
Prior-standardized AP evaluated every sample at $\pi_\star=0.06373$ and gave corresponding values of $0.180$ and $0.158$. The small residual movement reflects sampling variation in the empirical class-conditional score distributions. Chance normalization placed these results on the common effect scale used by the support functional.
...
Panel B displays their empirical replication distributions. The circles mark the replication means, and the diamonds mark supported chance-normalized AP.
...
\subsection{Conditional calibration and sparse outcomes}

Population random ranking has $\CNAP_{\pi_\star}=0$, but the supported chance level varied materially with positive count and score resolution. At five positive outcomes, $\SAPnullHat$ ranged from $0.004$ for two score levels to $0.045$ for continuous scores; at 100 positives, it ranged from $-0.001$ to $0.003$. Conditional-null calibration restored the mean null value to approximately zero across four real-score designs.

As the positive count increased from 5 to 100, the median complete adjustment $\Tpi(D_{\mathrm{obs}})-\SAPcalHat$ declined from $0.050$ to $0.008$. Its 10th to 90th percentile interval narrowed from $0.038$--$0.091$ to $0.006$--$0.012$. Appendix~\ref{app:bank-diagnostics} gives the design grid, calibration distributions, reporting sequence, and computational details. Population-level interval performance still requires simulation, and the observed trend need not hold under every score distribution, tie pattern, or sampling design.
...
Predictive-performance reporting supports two tasks: interpreting the evidence for a model considered on its own and informing a choice between models evaluated together. Both use the same performance scale and replication framework. Each calls for a summary matched to its evidential purpose.

For average precision, reference prevalence $\pi_\star$ gives selected-set purity a common target-population interpretation, and chance normalization places population random ranking at zero and perfect ranking at one. For any effect $T$ oriented so that larger values are better,
\[
S_K(T;\Regime)
=
\E_{\Regime}\left[\min(T_1,\ldots,T_K)\right].
\]
Each execution of $\Regime$ gives the claim an independent opportunity for defeat. The supported value is the average maximal claim that survives all $K$ executions. At the default $K=2$,
\[
S_2(T;\Regime)
=
\E_{\Regime}[T]
...
\;.
\]
The disagreement between replications lowers the retained claim. Increasing $K$ gives the claim more opportunities to fail. This Popperian severity remains relative to the regime, which declares the environments, mechanisms, and replication counts allowed to vary.

For a fixed model evaluated on held-out outcomes, the canonical stratified bootstrap estimates its replication distribution. Permuting outcomes against fixed scores measures the finite-design chance level while preserving the class counts and tie structure. The calibrated score
\[
\SAPcalHat
=
\frac{\SAPHat-\SAPnullHat}{1-\SAPnullHat}
\]
assigns that level zero and perfect separation one. It describes how much performance the model retains relative to the chance level of its evaluation design.

Separately reported calibrated values can support cautious benchmarking when studies share a reference prevalence, AP convention, replication severity, compatible regimes, and comparable validation designs. These values remain marginal and data-conditional. They describe the supported performance of each evaluated model without identifying the advantage that would arise under a common evaluation.

When both models are available on the same units, supported superiority applies the operator to their within-replication difference:
\[
S_2(\Delta_{\pi_\star};\Regime)
\]
This pairing allows shared changes in evaluation difficulty to cancel. Subtracting marginal supported values generally overstates the retained advantage, and model-specific conditional calibration can change model ordering.

A candidate meets the performance-based replacement criterion when
\[
S_2(\Delta_{\pi_\star};\Regime)>d.
\]
The margin $d$ states the smallest retained advantage that would justify replacement. An outer lower confidence bound above $d$ supplies population-level evidence for this performance claim. Costs, feasibility, fairness, calibration, and operational consequences may also enter a deployment decision.

Every result inherits the scope of its regime. Claims involving new sites, time periods, populations, or development runs require data and resampling procedures that represent those sources of variation. Repeated-development claims also require a null construction for the full fitting process. Simulation remains needed for interval coverage, sparse outcomes, ties, centering choices, and sensitivity to a misspecified regime.

Conditionally calibrated supported performance gives a model an interpretable finite-design reference. Compatible marginal reports allow cautious benchmarking when joint predictions are unavailable. Paired supported superiority supplies performance evidence for a model-choice or replacement decision when models share an evaluation.
...
\subsection{Threshold behavior across reference prevalences}
\label{app:reference-behavior}

At threshold $k$, the chance-normalized contribution is
\[
\frac{\pi_\star(r_k-f_k)}
{\pi_\star r_k+(1-\pi_\star)f_k}.
\]
If $f_k>0$, this contribution converges to zero as $\pi_\star\to0$. If $f_k=0$ and $r_k>0$, it equals one for every $\pi_\star$. Low reference prevalences therefore emphasize the part of the ranking that retrieves positives before any false positive enters.
...
\end{equation}
VaThe right-hand side is one minus the reciprocal of the likelihood ratio in Equation~\eqref{eq:odds}. The contribution changes monotonically with $\pi_\star$ whenever its threshold lies entirely on one side of the ROC diagonal:
\begin{equation}
...
\section{Implementation details for evaluation-generating regimes}
\label{app:regime-details}

Independence applies to complete replications. It need not hold among observations within a replication. Writing $C$ for the contextual variables allowed to vary, a hierarchical regime draws
\[
C^{(j)}\sim P_{\Regime}(C),
\qquad
D^{(j)}\sim P_{\Regime}(D\mid C^{(j)}).
\]
Two evaluations can therefore be independent executions of $\Regime$ while units within a site or measurements within a batch remain dependent.

The evaluation unit receives a score and an outcome. The exchangeable unit is resampled or permuted as a whole. They coincide when evaluation units are independent. Under nesting, the exchangeable unit is the cluster, and the evaluation units inside it move together.

Repeated development requires access to the training data and the complete fitting pipeline. Every replication entails a training run. A development-level regime can define and estimate $S_K$. Its conditionally calibrated form requires a null construction for the development process.
...
\label{app:joint-survival-proof}

Let $M_K=\min(T_1,\ldots,T_K)$ for independent draws from a distribution supported on $[L,U]$. For each realized $m\in[L,U]$,
\[
m
=
L+\int_L^U\mathbf{1}\{m>s\}\,ds.
\]
Taking expectations gives
\[
\E[M_K]
=
L+\int_L^U\Pr(M_K>s)\,ds.
\]
The event $M_K>s$ occurs exactly when every replication exceeds $s$. Independence gives
\[
\Pr(M_K>s)=\Pr(T>s)^K,
\]
which yields Equation~\eqref{eq:joint-survival}.
...
\end{equation}
Here $\pi$ in the denominator is the circle constant. Under normality, $S_2$ equals the mean shifted down by $0.564$ replication standard deviations and lies at approximately the $29$th percentile of that distribution.

This numerical resemblance to a one-sided lower bound does not make $S_2$ an uncertainty interval. The operator averages joint survival under the declared regime, while a confidence bound addresses uncertainty in estimating a parameter. The normal approximation can also misstate the penalty for skewed or discrete replication distributions. Sparse positives and heavy ties often produce those shapes. The exact minimum form requires only a finite first moment.
...
The parameter $K$ specifies the number of independent tests a performance claim must survive~\cite{popper1959logic}. Severity here means exposure to potentially falsifying tests. It does not denote Mayo's error-statistical severity or carry an error-probability interpretation~\cite{mayo1996error}. The regime determines how demanding each test is, so $K$ controls one aspect of severity within a declared regime.

For effects bounded by $L$ and one,
\[
S_K
=
L+\int_L^1\Pr(\Theta_{\pi_\star}>s)^K\,ds .
\]
Increasing $K$ lowers $S_K$ for any nondegenerate distribution. Its limit as $K\to\infty$ is the essential infimum of $\Theta_{\pi_\star}$. A finite sample cannot estimate this strict criterion reliably because its empirical estimate reduces to the worst available replicate. The default $K=2$ is the smallest replication count that tests joint survival.
...
\subsection{Calibration anchors and score resolution}
\label{app:calibration-details}

A score vector whose ties mix outcomes cannot separate the classes, so no assignment of outcomes to it reaches $\Tpi=1$. For example, a five-level score with $n=200$, $n_+=20$, and $\pi_\star=0.10$ attains at most $0.44$. Normalization by this attainable maximum would remove the shortfall from the reported value.

The calibration in Equation~\eqref{eq:calibrated} retains perfect separation as the common upper anchor. Score resolution is part of the information supplied by a model, and a score-specific upper anchor would change the scale across score representations. Coarsening therefore lowers $\SAPHat$ and $\SAPcalHat$. This treatment charges both genuine loss of resolution and avoidable loss caused by rounding, which motivates the use of unrounded scores in Appendix~\ref{app:prevalence}.

\subsection{Validity of conditional-null calibration}
\label{app:conditional-validity}

The permutation distribution represents the sharp null $Y\perp S$ conditional on the realized scores and permitted outcome assignments. It does not test the weaker statement $\CNAP_{\pi_\star}=0$. A systematically reversed ranker can have $\CNAP_{\pi_\star}<0$ while violating the sharp null.

The permutation holds the scores fixed and moves the outcomes. Its validity requires that no evaluation outcome enter the mechanism producing the scores. A held-out test set satisfies this condition. Ordinary cross-fitting generally does not: a score in one fold depends on outcomes in other folds, so the full outcome vector need not be exchangeable given the full score vector. A cross-fitted analysis requires a permutation design that preserves this dependence, often by rerunning the fitting procedure. The same condition excludes scores from a model whose tuning, feature selection, operating threshold, or stopping rule used the evaluation outcomes.

A regime that reruns model development can define and estimate $S_K$. The conditional-null calibration developed here does not assess that development process. Such a regime requires a null construction for the full process before a calibrated value can be reported.
...
\subsection{Interval considerations for supported superiority}
\label{app:superiority-intervals}

The disagreement penalty can skew the supported-superiority estimator, and the minimum kernel is nondifferentiable at equal arguments. Sparse positive counts and heavy ties increase discreteness near zero. Studentized or bias-corrected intervals may improve on percentile intervals. Their coverage requires simulation under the evaluation designs and replication regimes in which they will be used.
...
The final diagnostic changes both the observed counts and the replication counts inherited by the canonical bootstrap. Its rows represent different same-design corroboration questions and do not estimate one fixed prospective target.
