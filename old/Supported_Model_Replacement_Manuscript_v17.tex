...
hand. Its verdict is conjunctive: the retained advantage must exceed a
prespecified magnitude threshold, and the advantage must clear a prespecified
floor across a sufficient fraction of the declared tests. The evidence is
aggregated only after each declared test has challenged the candidate on its
own, so a gain in one place cannot offset a shortfall in another. Its verdict is
...
ProteinGym, a collection of 91 protein mutation-effect assays, we apply the rule
to comparisons of released protein language models (ESM-1v, ESM-2) with an
established evolutionary model (EVE).
\end{abstract}
...
tie-break is specialized to PR/CNAP.
...
\subsection{Operational choice after fixed-policy graph reduction}
\label{app:multi-system-operational}

The fixed-policy graph reduction determines the admissible set $S_0$.
If $S_0$ is empty, there is no eligible choice. If it contains one system,
that system is selected. When several systems remain, observed ranking
performance supplies an operational comparison among them. Both eligibility
and the performance benchmark are restricted to $S_0$.

Let $D_1,\ldots,D_M$ be the empirical evaluations declared for this comparison.
Their identities are retained, and all surviving systems are evaluated on the
same units within each evaluation. Define
\[
\widehat{\pi}_m=\frac{N_{m,+}}{N_m},
\qquad
q_m(B)=\CNAPta_{B,\widehat{\pi}_m}(D_m),
\qquad B\in S_0.
\]
The quantity $q_m(B)$ is tie-averaged empirical AP expressed on CNAP's
random-to-perfect opportunity scale, using the tie convention of
Equation~\eqref{eq:tie-average}. It evaluates performance at the observed
class composition.

The observed-CNAP regret of system $B$ in evaluation $m$ is
\begin{equation}
r_m(B)
=
\max_{C\in S_0}q_m(C)-q_m(B),
\qquad B\in S_0.
\label{eq:observed-cnap-regret}
\end{equation}
Regret is the shortfall from the best-performing survivor in that evaluation.
It is nonnegative and equals zero when $B$ matches the best survivor.
Different systems may supply the benchmark in different evaluations.

First minimize the largest regret. Among systems tied on that value, minimize
the second-largest regret, then the third-largest, continuing until a
distinction appears or the complete vectors coincide. This
\emph{lexicographic minimax CNAP-regret rule} compares the descending vectors
\[
\mathbf r^\downarrow(B)
=
\operatorname{sort}_{\mathrm{descending}}
\bigl(r_1(B),\ldots,r_M(B)\bigr).
\]
With $\leq_{\mathrm{lex}}$ denoting lexicographic order, the operational set is
\begin{equation}
S_{\mathrm{op}}
=
\left\{
B\in S_0:
\mathbf r^\downarrow(B)\leq_{\mathrm{lex}}\mathbf r^\downarrow(C)
\text{ for every }C\in S_0
\right\}.
\label{eq:operational-cnap-selection}
\end{equation}
Identical complete vectors remain tied.

For example, suppose $S_0=\{A,B\}$ and two evaluations give:
\begin{center}
\begin{tabular}{@{}lrrc@{}}
\toprule
System & CNAP in $D_1$ & CNAP in $D_2$ & Descending regret vector \\
\midrule
$A$ & $0.90$ & $0.70$ & $(0.08,0)$ \\
$B$ & $0.85$ & $0.78$ & $(0.05,0)$ \\
\bottomrule
\end{tabular}
\end{center}
The benchmarks are $0.90$ in $D_1$ and $0.78$ in $D_2$.
System $B$ is selected because its largest shortfall is $0.05$, compared with
$A$'s $0.08$.

The rule minimizes the most serious remaining observed relative deficiency
among the admissible systems. A favorable result elsewhere cannot compensate
for a larger worst regret. The condition and computational challenges
determine admissibility; operational selection compares performance at the
evaluations' observed prevalences. The operational result leaves the
fixed-policy survivor set unchanged.
...
\begin{quote}
Under the declared policy, the admissible set was $S_0=[\ldots]$.
Observed-CNAP regret selected [system or tied set], with descending regret
vector [values]. For [selected system], the largest regret was [value] in
[evaluation], relative to [surviving comparator].
\end{quote}

\subsection{Criticism-first operational choice after fixed-policy graph reduction}
\label{app:multi-system-operational-criticism}

The preceding reductions answer which systems remain scientifically
...
Take the complete roster $V=\{A,B,C,D\}$. After all planned pairwise
assessments and the prespecified context reduction, suppose the fixed-policy
graph at $\gamma_0=0.81$ contains the edges
\[
A\longrightarrow D,
\qquad
B\longrightarrow D,
...
incoming edges, while $\{D\}$ does. The fixed-policy reduction therefore gives
$S_0=\{A,B,C\}$. System $D$ cannot participate in the operational ordering,
...
threshold, challenge orders, condition set, computational procedure, and every
...
requirements that would support the direction. In the common one-evaluation
boundary with observed anchor above $d$, $J$ computational effects, and
$K_C=2$, it is
\[
w_{m,e}(A\to B)
=
\frac{\binom{R_{m,e,0}}{2}}{\binom{J}{2}},
\]
provided that the observed and full supported magnitudes exceed $\delta$.
