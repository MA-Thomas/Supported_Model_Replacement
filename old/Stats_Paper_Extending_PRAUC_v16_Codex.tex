...
Complementary empirical illustrations apply the projected path to one temporal
Bank Marketing evaluation and the observed path to 91 ProteinGym assays. The
latter comparison finds no stable replacement ordering among EVE, ESM-1v, and
ESM-2 under the declared assessment.
\end{abstract}
...
Section~\ref{sec:illustrations} later contrasts a one-evaluation computational
projection with an observed finite list of DMS assay effects. The examples are
used to expose the operator's mechanics and evidential scope, not to infer a
distribution of unseen evaluations.
...
For $K=2$, each subset is a pair of complete effect-list entries. If the list
contains observed assay effects, the pair contains two assays; if it contains
projected computational effects, the pair contains two computational
replications. It is not a pair of data points within an evaluation. A pair
survives only when both of its retained effects exceed $d$. Thus a requirement
near $0.9^2=0.81$ corresponds approximately to requiring nine tenths of the
individual effects to clear the floor, with the exact finite fraction given by
Equation~\eqref{eq:empirical-survival-fraction}. This is a policy threshold on
the observed finite list, not a confidence level.
...
Two empirical illustrations exercise the two evidence paths without changing
the interpretation of the operator. The Bank Marketing illustration begins
with one observed evaluation and constructs a projected list of computational
effects. The ProteinGym illustration instead applies the operator directly to
a finite list of observed assay effects. In both studies the condition set is
the target-prevalence interval $\PSet=[0.01,0.50]$, the challenge order is
$K=2$, and the two-part rule uses $\delta=d=0$ and $\gamma=0.81$. These values
define the illustrated policy; they are not proposed as defaults.

\subsection{One evaluation and a computational projection: Bank Marketing}
\label{sec:bank-illustration}

The UCI Bank Marketing data record ordered direct-marketing contacts and
whether each customer subscribed to a term deposit \cite{moro2014bank}. The
first 32,950 of 41,188 records were used to fit the models, and the final 8,238
formed one temporal evaluation. Contact duration was excluded before fitting
because it is unavailable at the decision to initiate the contact. The primary
comparison was random forest against a logistic model using the available
pre-contact predictors. A full logistic model against a demographic-only
logistic model supplied a positive control. The models and the split serve to
illustrate the assessment rather than to make a claim about banking practice.

Figure~\ref{fig:bank-ap-cnap} holds the fitted full-logistic ranking and its
observed class-conditional score behavior fixed while varying only the declared
target prevalence. AP changes with that composition. CNAP retains the profile
but places population random ranking at zero throughout the interval.
...
declared target-prevalence interval. The vertical line gives the training
prevalence as a reference, not as a preferred target. Varying prevalence holds
the observed class-conditional ranking behavior fixed.}
\label{fig:bank-ap-cnap}
\end{figure}

The paired difference must be formed at each prevalence before the weakest
condition is selected. Figure~\ref{fig:bank-paired-profile} shows why. The
random-forest ordering changes relative to the full logistic model inside the
declared interval, so its retained effect is negative even though it is
positive at lower prevalences. The positive-control difference remains
positive throughout the interval.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript_images/banking-paired-prevalence-profile.pdf}
\caption{Paired CNAP differences in the single temporal evaluation. Each
orange point is the infimum over the complete declared prevalence interval.
The limiting prevalence is allowed to differ between comparisons.}
\label{fig:bank-paired-profile}
\end{figure}

The temporal holdout supplies $M=1$ observed full evaluation, so it cannot
supply observed order-2 support. We instead drew $J=600$ paired,
class-stratified computational replications from its score pools, preserving
the observed evaluation counts of 2,540 subscribers and 5,698
non-subscribers. Every replication used the same resampled units for both
models and throughout the prevalence profile. Figure~\ref{fig:bank-projection}
shows the resulting projected list for the positive control. The prevalence
infimum is taken within each complete replication before order-2 support is
applied across replications.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript_images/banking-projected-support.pdf}
\caption{Projected support from one observed evaluation. Selected curves in
panel A are complete paired computational replications; each dot is that
replication's retained effect. Panel B collects all 600 retained effects and
contrasts their mean with order-2 supported magnitude. These replications do
not constitute additional observed evaluations.}
\label{fig:bank-projection}
\end{figure}

For random forest against full logistic regression, projected order-2 support
was $-0.0104$ and 114 of 600 effects cleared zero, giving literal survival
$0.036$ and no verdict. For full against demographic-only logistic regression,
projected support was $0.0130$ and all 600 effects cleared zero, giving literal
survival $1.000$ and a supported-replacement verdict. The reversed orientation
did not satisfy the rule in either comparison. Figure~\ref{fig:bank-decision}
keeps the magnitude and survival requirements separate.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript_images/banking-two-part-decision.pdf}
\caption{The two-part projected decision in the Bank Marketing illustration.
Replacement requires both supported magnitude to exceed $\delta=0$ and
literal order-2 survival to exceed $\gamma=0.81$. Failure returns no verdict,
not a finding that the incumbent is superior.}
\label{fig:bank-decision}
\end{figure}

The same fitted score pools also illustrate the AUROC extension in
Appendix~\ref{app:auroc}. Pure prior change supplies no AUROC challenge, so the
companion calculation increases the allowed within-class concentration on
represented cases. Random forest against full logistic regression failed the
policy at the observed mix. The positive control passed at $\Gamma=1$ and
first failed in the numerical interval $1.10<\Gamma^\dagger\leq1.20$, as shown
in Figure~\ref{fig:bank-auroc}. The interval is a numerical bracket, not a
confidence interval.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript_images/banking-auroc-case-mix-breakdown.pdf}
\caption{Projected empirical case-mix breakdown for the banking score pools.
Larger $\Gamma$ permits greater concentration on fewer represented cases. The
challenge cannot introduce customer types absent from the temporal
evaluation.}
\label{fig:bank-auroc}
\end{figure}

\subsection{Several observed evaluations: ProteinGym}
\label{sec:proteingym-illustration}

ProteinGym provides standardized deep-mutational-scanning assays together with
released model scores \cite{notin2023proteingym}. We used version 1.3
substitution data, retained assays with manual biological cutoffs, and retained
single amino-acid substitutions. Experimentally nonfunctional or deleterious
variants formed the positive class. Because the released scores are oriented
toward fitness, they were negated before analysis so that larger values favored the
positive class. The three primary paired comparisons were ESM-1v ensemble
against EVE ensemble, ESM-2 650M against ESM-1v ensemble, and ESM-2 650M
against EVE ensemble. These scores come from already released models; no model
was fitted or tuned for this study \cite{frazer2021eve,meier2021esm1v,lin2023esm2}.

The aligned cohort contained 277,873 single substitutions in 91 assays,
including 96,987 deleterious and 180,886 functional variants. Every retained
variant had finite predictions from all compared models. Each assay supplied
one full evaluation effect: the paired CNAP difference was computed throughout
$[0.01,0.50]$, and its infimum was retained. No computational resampling was
performed. The resulting $M=91$ effects therefore enter the observed path
directly.

Figure~\ref{fig:proteingym-effects} shows that the retained effects are
heterogeneous for all three comparisons. The plots order assays separately
within each comparison; horizontal position does not match the same assay
between panels.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{manuscript_images/proteingym-observed-assay-effects.pdf}
\caption{Observed retained assay effects for the three primary ProteinGym
comparisons. Each point is one complete DMS assay after its paired model
difference has faced the full declared prevalence interval. Green and orange
regions distinguish positive from nonpositive retained effects; they are not
uncertainty bands.}
...
\begin{table}[htbp]
\centering
\caption{Observed order-2 ProteinGym results. All comparisons use the same 91
assays and the same declared policy.}
\label{tab:proteingym-results}
\begin{tabularx}{\textwidth}{@{}Xrrrr@{}}
\toprule
Candidate minus incumbent & Support & Assays $>0$ & Pair survival & Verdict \\
\midrule
ESM-1v ensemble minus EVE ensemble & $-0.1292$ & $50/91$ & $0.299$ & No verdict \\
ESM-2 650M minus ESM-1v ensemble & $-0.1048$ & $40/91$ & $0.190$ & No verdict \\
ESM-2 650M minus EVE ensemble & $-0.1264$ & $56/91$ & $0.376$ & No verdict \\
\bottomrule
\end{tabularx}
\end{table}

Here an order-2 pair means two complete assay evaluations, not two protein
variants. There are $\binom{91}{2}=4{,}095$ distinct assay pairs, and a pair
survives at $d=0$ only when both assays have positive retained effects. The
strict requirement $\psi_{2,91,0}>0.81$ therefore requires at least 82 of 91
assays to clear zero, since
$\binom{82}{2}/\binom{91}{2}=0.811>0.81$. None of the comparisons approached
that count. ESM-2 exceeded EVE in 56 assays, for example, but the remaining
losses were sufficiently severe that both mean retained effect and order-2
support were negative. Thus a majority win count did not support replacement.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{manuscript_images/proteingym-two-part-decision.pdf}
\caption{Observed supported magnitude and literal assay-pair survival for the
three primary ProteinGym comparisons. The right panel's bars are exact
fractions of the 4,095 observed assay pairs whose two retained effects both
exceed zero.}
\label{fig:proteingym-decision}
\end{figure}
\clearpage

All three forward comparisons and all three reversed comparisons returned no
verdict. The result does not establish a reverse hierarchy. It shows that the
three released models do not form a stable replacement ordering across these
assay contexts under the declared prevalence and order-2 challenges.

The evaluation regime treats a DMS assay as the full evaluation unit. The 91
assays represent 80 UniProt proteins and 60 publications; 20 assay rows share
a protein with another retained assay, and 42 come from publications that
contribute more than one assay. Those multiplicities are retained and reported
rather than relabeled as independent research programs. Consequently, the
claim is limited to the finite assay-level collection. The present primary
analysis does not reduce the collection to one assay per publication.
...
The empirical illustrations show the distinction directly. In the banking
study, the positive control clears both projected requirements relative to one
temporal evaluation, while the close comparison returns no verdict. In the
ProteinGym study, the observed assay list supplies genuine contextual
heterogeneity: all three model comparisons return no verdict even when one
candidate has a positive retained effect in a majority of assays. Win counts
and marginal averages therefore do not replace the magnitude and persistence
questions posed by the support operator.
