...
const OUT_DIR = "/Users/thomm15/Documents/Stats_Paper_AUPRC_extension/.tmp/prauc_deck_v12/rendered";
const FINAL_PPTX = "/Users/thomm15/Documents/Stats_Paper_AUPRC_extension/Stats_Paper_Extending_PRAUC_v12_slides.pptx";
...
function addChrome(slide, title, section, page, accent = C.blue) {
  ...
  addText(slide, `section-${page}`, section.toUpperCase(), { left: 42, top: 28, width: 600, height: 24 }, {
    fontSize: 14, bold: true, color: accent,
  });
  addText(slide, `title-${page}`, title, { left: 42, top: 60, width: 1196, height: 68 }, {
    fontSize: 38, bold: true, color: C.dark, autoFit: "shrinkText",
  });
  addShape(slide, `rule-${page}`, "rect", { left: 42, top: 145, width: 1196, height: 2 }, accent);
...
function addTitleSlide(presentation, page) {
  ...
  slide.background.fill = C.white;
  addText(slide, "title-eyebrow", "METHODOLOGICAL PROPOSAL", { left: 42, top: 42, width: 640, height: 32 }, {
    fontSize: 18, bold: true, color: C.blue,
  });
  addText(slide, "deck-title", "Supported model comparison\nacross target prevalences", {
    left: 42, top: 180, width: 1030, height: 250,
...
function addThreeColumnSlide(presentation, cfg, page) {
  ...
  cfg.items.forEach((item, i) => {
    ...
    addText(slide, `num-${page}-${i}`, item.kicker ?? `0${i + 1}`, { left: xs[i] + 30, top: 235, width: 70, height: 28 }, {
      fontSize: 15, bold: true, color: cfg.accent ?? C.blue,
    });
...
function addDividerSlide(presentation, cfg, page) {
  ...
  addText(slide, `divider-kicker-${page}`, cfg.kicker.toUpperCase(), { left: 42, top: 44, width: 600, height: 32 }, {
    fontSize: 18, bold: true, color: cfg.accent ?? "#7DB4FF",
  });
  addText(slide, `divider-title-${page}`, cfg.title, { left: 42, top: 245, width: 1050, height: 250 }, {
...
addTwoColumnSlide(presentation, {
  title: "The same ranking can deliver a very different selected set",
  section: "The problem",
  accent: C.coral,
  leftFill: C.tealSoft,
...
addThreeColumnSlide(presentation, {
  title: "A performance claim needs three declared challenges",
  section: "The problem",
  accent: C.blue,
  items: [
...
addFormulaSlide(presentation, {
  title: "Prior standardization changes the population, not the ranking",
...
addFormulaSlide(presentation, {
  title: "Prior odds and ranking evidence separate cleanly",
  section: "Problem I · Prior standardization",
  accent: C.blue,
...
addTwoColumnSlide(presentation, {
  title: "Reweighting is valid only under score-level invariance",
...
addFormulaSlide(presentation, {
  title: "CNAP puts every prevalence on a random-to-perfect scale",
...
addLineChartSlide(presentation, {
  title: "A robust claim keeps the lowest point of the prevalence profile",
...
addThreeColumnSlide(presentation, {
  title: "A replication regime defines what is actually redrawn",
  section: "Problem II · The evaluation-generating regime",
  accent: C.coral,
  items: [
    { kicker: "FIXED MODEL", head: "New evaluation units", body: "Hold environment, measurement, and fitted model fixed.", fill: C.gray },
    { kicker: "NEW ENVIRONMENT", head: "Release site conditions", body: "Redraw environments; declare how measurement varies.", fill: C.coralSoft },
    { kicker: "REPEATED DEVELOPMENT", head: "Rerun the procedure", body: "Redraw training, tuning, selection, fitted model, and evaluation.", fill: C.goldSoft },
  ],
  bottom: "A unit bootstrap identifies only the first regime.",
  source: "Subsection “The evaluation-generating regime”",
}, p++);

addTwoColumnSlide(presentation, {
  title: "One assessment uses two different modes of challenge",
...
addFormulaSlide(presentation, {
  title: "Joint survival uses the minimum—not the average",
...
addThreeColumnSlide(presentation, {
  title: "Projected support is not a count of completed replications",
  ...
  items: [
    ...
    { kicker: "NEW DATA", head: "Empirical replication", body: "New environments or development runs can expose failures the bootstrap cannot see.", fill: C.tealSoft },
  ],
...
addFormulaSlide(presentation, {
  title: "One estimand accumulates both challenges",
...
addTwoColumnSlide(presentation, {
  title: "The prevalence search must happen inside each replication",
...
addThreeColumnSlide(presentation, {
  title: "A low supported value has two distinct diagnostic costs",
  section: "Problem III · Why operator order matters",
  accent: C.coral,
  items: [
...
addFormulaSlide(presentation, {
  title: "More challenges cannot improve raw support",
...
addProcessSlide(presentation, {
  title: "Bootstrap one joint prevalence profile",
  section: "Estimating support · The fixed-model bootstrap",
  accent: C.blue,
  lead: "Use this regime only when evaluation units are the mechanism being redrawn.",
  steps: [
...
addFormulaSlide(presentation, {
  title: "A grid is exact only when the declared challenge is finite",
...
addTwoColumnSlide(presentation, {
  title: "Population chance and finite-design chance are not the same",
...
addFormulaSlide(presentation, {
  title: "A one-model report can be calibrated to its realized design",
  ...
  why: "Report raw support, the anchor, and the ratio together. Different score vectors define different affine scales, so calibrated values cannot be ranked or subtracted.",
  source: "Subsection “A design-relative report for one model”",
}, p++);

addTwoColumnSlide(presentation, {
  title: "Marginal reports compare adverse cases that may never coincide",
...
addTwoColumnSlide(presentation, {
  title: "Ordinary pairing still rewards arbitrary score refinement",
...
addFormulaSlide(presentation, {
  title: "Tie-averaged AP removes expected credit for arbitrary ordering",
  ...
  meaning: "Imagine outcome-blind jitter that reorders only within ties, then average the resulting AP.",
  why: "Useful refinement still earns credit when it orders outcomes well. Granularity alone earns zero expected advantage.",
  source: "Subsection “Tie-averaged average precision”",
}, p++);

addFormulaSlide(presentation, {
  title: "The replacement estimand forms the paired difference first",
...
addThreeColumnSlide(presentation, {
  title: "A no-positive-anchor guarantee needs exchangeable labels",
  section: "Problem V · Reference behavior and design scope",
  accent: C.gold,
  items: [
    { kicker: "REFERENCE LAW", head: "All fixed-count label vectors are equally likely", body: "Condition on both score vectors and n+.", fill: C.goldSoft },
    { kicker: "CONSEQUENCE", head: "Expected paired profile is zero", body: "Tie structure no longer changes the expected AP anchor.", fill: C.tealSoft },
...
addFormulaSlide(presentation, {
  title: "A coherent procedure can return no verdict",
...
addThreeColumnSlide(presentation, {
  title: "The number means nothing without its assessment",
...
addThreeColumnSlide(presentation, {
  title: "Report the claim, its design, and the diagnostics separately",
  section: "Interpretation · What should be reported",
  accent: C.teal,
  items: [
    { kicker: "CLAIM", head: "Supported quantity + assessment", body: "Raw value; Π★ and its basis; ℛ; K; replication counts and resampling unit.", fill: C.tealSoft },
    { kicker: "DECISION", head: "Pairing + practical margin", body: "Orientation, tie averaging, exchangeability argument, d, and reversed orientation.", fill: C.blueSoft },
    { kicker: "EVIDENCE", head: "Diagnostics + uncertainty", body: "Profiles, minimizing prevalences, costs, computation, Monte Carlo error, and validated outer bound.", fill: C.gray },
  ],
  source: "Subsection “What should be reported”",
}, p++);

addTwoColumnSlide(presentation, {
  title: "Sparse positives make the whole supported profile coarse",
...
addThreeColumnSlide(presentation, {
  title: "Four tempting conclusions are outside the method’s license",
  section: "Interpretation · Inferences the result does not license",
  accent: C.coral,
  items: [
    { kicker: "TRANSPORT", head: "Beyond prior shift", body: "Reweighting does not protect against within-class behavior changes.", fill: C.coralSoft },
    ...
    { kicker: "DEPLOYMENT", head: "Good ranking implies intervention benefit", body: "Passive evaluation does not identify effects of acting on the ranking.", fill: C.blueSoft },
  ],
...
addFormulaSlide(presentation, {
  title: "Support is the magnitude that survives declared challenges",
  section: "Conclusion",
  accent: C.blue,
  latexLines: [
    String.raw`Z_j=\inf_{π∈Π_{∗}}\operatorname{CNAP}_π(D_j)`,
  ...
  meaning: "Name the populations, the replication law, and the number of independent opportunities for defeat.",
  why: "For replacement: compare on the same units, average over arbitrary tie orderings, and require the outer lower bound to clear a practical margin.",
  source: "Conclusion",
}, p++);

addDividerSlide(presentation, {
  kicker: "Technical appendix",
  title: "Definitions, proofs,\nand computation",
  subtitle: "One slide per appendix subsection or stand-alone technical section.",
  accent: "#7DB4FF",
...
addFormulaSlide(presentation, {
  title: "The CNAP profile can have an endpoint—or an interior—minimum",
...
addProofSlide(presentation, {
  title: "The operator gap appears only after expectation",
...
addProofSlide(presentation, {
  title: "Challenge monotonicity is a pathwise inclusion argument",
...
addFormulaSlide(presentation, {
  title: "Increasing K shifts weight toward the lower tail",
...
addFormulaSlide(presentation, {
  title: "Independent joint survival selects a power distortion",
...
addProofSlide(presentation, {
  title: "Marginal subtraction overstates paired advantage in two steps",
...
addProofSlide(presentation, {
  title: "Refinement neutrality is a tower-property argument",
...
addProofSlide(presentation, {
  title: "Exchangeable labels make the reference law rank-invariant",
...
addTwoColumnSlide(presentation, {
  title: "Restricted label symmetry creates multiple design orbits",
...
addTwoColumnSlide(presentation, {
  title: "A null anchor is not an additive correction",
...
addFormulaSlide(presentation, {
  title: "Orientation compares an expected minimum with a maximum",
...
addProcessSlide(presentation, {
  title: "Tie-averaged AP has a closed form—no enumeration required",
...
addFormulaSlide(presentation, {
  title: "Sorted replication effects yield the complete order-K estimator",
...
addFormulaSlide(presentation, {
  title: "Monte Carlo error uses leave-one replication projections",
...
addThreeColumnSlide(presentation, {
  title: "Efficient prevalence evaluation reuses structure",
  section: "Appendix F · Prevalence evaluation",
  accent: C.gold,
  items: [
    { kicker: "FINITE SET", head: "Reuse threshold counts", body: "Ordinary block AP costs O(BGH) for B bootstraps, G prevalences, H thresholds.", fill: C.goldSoft },
    { kicker: "PAIRED TIES", head: "Reuse block combinatorics", body: "Only the prevalence-dependent precision term changes.", fill: C.tealSoft },
    { kicker: "INTERVAL", head: "Optimize one dimension", body: "Check endpoints and stationary points; validate against a dense grid.", fill: C.blueSoft },
  ],
  source: "Appendix subsection “Prevalence evaluation”",
}, p++);

addProcessSlide(presentation, {
  title: "Outer bounds must repeat the entire inner procedure",
