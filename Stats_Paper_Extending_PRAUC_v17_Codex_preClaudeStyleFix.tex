...
finely, even when that refinement is unrelated to the outcomes. The next
section constructs one retained paired AP effect before any empirical or
computational effects are combined.
...
condition. We first construct one such effect from one full evaluation.
...
The operations defining one entry are now complete:
\[
\begin{aligned}
&\text{pair the models}
\longrightarrow
...
\end{aligned}
\]
The next section asks how observed anchors and computational effects should be
combined without erasing the scientific identity of the indices that produced
them.
...
We begin with the finite-list operator that supplies the foundation of the
hierarchy.
...
nested challenges is positive. It does not mean that a randomly drawn unseen
evaluation has a positive advantage.
...
The hierarchy has now specified what survives. The next section states how
much retained magnitude and literal survival a replacement policy requires.
...
The decision rule is now complete. The next section states what counts as
empirical evidence and how the required finite-array quantities are generated
and computed.
...
An admitted empirical evaluation is an \emph{empirical replication} only
under the stronger conditions that it was separately conducted, its units do
not overlap those in another replication, and its model construction and data
collection were not altered in response to another evaluation in the same
set. Resampling draws, administrative partitions, cross-validation folds, and
repeated analyses of the same units do not create empirical replications.
...
These calculations implement the staged rule without changing the evidential
identity of any array entry. The illustrations now show where two empirical
comparisons stop in that sequence.
...
class-conditional score behavior. It changes the target class prior; it does
not remove or transport differences in protein, measurement process,
experimental context, or within-class score behavior across assays.
...
observed-only. It remains a finite-evidence robustness result over represented
cases, not a confidence statement about unseen cases or contexts.
