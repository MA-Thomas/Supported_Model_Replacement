...
hand. Within each context, its verdict is conjunctive: the retained advantage must exceed a
prespecified magnitude threshold, and the advantage must clear a prespecified
floor across a sufficient fraction of the declared tests. Each complete challenge is evaluated before averaging, so a favorable
component cannot offset a shortfall within that challenge. Its verdict is
what the observed data actually withstood, not an estimate of how the models
would compare on data not yet seen. We develop the rule for two widely used
ranking measures, average precision and AUROC.

Supported comparisons are assembled separately in each context.
Candidate-conservative reduction retains models that survive criticism in
every context; replacement-conservative reduction first retains only
replacement directions supported everywhere. These answer different questions
about what must persist across contexts.

On a bank-marketing benchmark, a deliberately
strong candidate withstands the tests while a close rival does not. On
ProteinGym, separate assessments of 91 protein mutation-effect assays support
context-specific replacements among ESM-1v, ESM-2, and EVE, but no model
remains admissible in every assay under either CNAP or AUROC. An appendix
retains the alternative joint assessment of one replacement claim across
...
Call $D_e$ a \emph{full empirical evaluation} when it contains the outcomes and
...
evaluations. A comparison that favors replacement in one collection of units
may fail in another context, so a claim intended to survive such tests should
declare the regime that admits them. Here a context identifies the scientific
circumstances in which an empirical evaluation tests the claim.
...
evaluation; and
\item a \emph{cross-context comparison} asks which systems remain admissible
after the supported comparisons in the admitted empirical evaluations have
been considered.
\end{enumerate}
Within each declared challenge, the noncompensation principle applies: each declared challenge is evaluated
...
Computational recombination can deepen the challenge applied to an empirical
evaluation, but it cannot repair an observed empirical failure. The observed
comparison therefore comes first. Only a claim that passes that comparison
is exposed to the additional within-evaluation computational challenge, and
the observed result remains part of every such challenge.

With several contexts, the analysis first establishes the supported
comparisons within each one. It then asks either which systems remain
admissible everywhere, or which remain after retaining only replacement
directions supported everywhere. The first rule allows different rivals to
expose a system's deficiencies in different contexts. The second requires
agreement on a successor. Section~\ref{sec:roster} develops these rules after
the context-specific replacement assessment has been defined.
...
will be. The following choices do this. The condition set $\CSet$ names the conditions
...
within each empirical evaluation. Finally, $K_C$ specifies the order of the computational challenge. We collect these choices in the
assessment specification
\[
\Assess
=
(\CSet,\Regime,\Boot,K_C).
\]
For a roster of systems, the cross-context reduction is also declared in
advance. This specification declares the attempted refutations represented by the
...
absent; Section~\ref{sec:context-properties} gives the corresponding formal boundary.
...
applied to it. The counts $N_e$, $M$, and $J_e$ describe what is available; the
order $K_C$ describes how many computational effects are jointly faced:
\begin{center}
...
\midrule
$N_e$ & units in empirical evaluation $e$ & evidence represented within that evaluation \\
$M$ & admitted empirical evaluations & empirical tests and contexts available \\
$J_e$ & computational draws within evaluation $e$ & accuracy with which the declared within-evaluation challenge is approximated \\
$K_C$ & computational challenge order & number of computational effects jointly faced within each empirical evaluation \\
\bottomrule
\end{tabularx}
\end{center}
Thus, increasing $J_e$ refines the computational approximation, whereas
increasing $K_C$ changes the challenge itself. Increasing $M$ adds empirical evidence; the cross-context rule determines
how its supported comparisons affect admissibility. No choice of $J_e$ turns one empirical
evaluation into several.

Appendix~\ref{app:joint-empirical} retains a distinct alternative: combining
the empirical evaluations within one replacement assessment. Its empirical
order $K_E$ defines that additional challenge. The main text instead preserves
each context's verdict before combining the supported comparisons.
...
We first construct the paired effect to which the replacement rule will apply.
...
advantage because they need not refer to the same prevalence. The same noncomposition persists in the joint empirical assessment of
Appendix~\ref{app:joint-empirical}; Appendix~\ref{app:support-proofs} gives
the result.
...
\end{equation}
Within empirical evaluation $D_e$, the observed retained effect is
\begin{equation}
z_e^{A:B,\mathrm{obs}}
=
\inf_{\pi\in\PSet}\Delta^{A:B}_\pi(D_e).
...
Let $D_e^{(j)}$ be computational replication $j$ generated from $D_e$ under
$\Boot$. Its retained effect is
\begin{equation}
z_{ej}^{A:B,\mathrm{comp}}
=
\inf_{\pi\in\PSet}\Delta^{A:B}_\pi(D_e^{(j)}).
...
This pipeline is written for average precision, but the context-specific rule that follows
...
assessment also reports a \emph{breakdown factor}: the smallest
$\Gamma$ at which the declared requirement first fails. Average precision and
AUROC thus enter one context-specific rule through different condition challenges.

\section{Observed-anchored replacement within a context}
\label{sec:context-rule}

Fix a context, suppress its index $e$, and consider an oriented claim $A:B$
on its empirical evaluation $D$. Its observed
retained effect $z^{\mathrm{obs}}$ has already faced every declared condition.
Suppose the declared procedure supplies $J$ computational effects,
$z_1^{\mathrm{comp}},\ldots,z_J^{\mathrm{comp}}$, to test dependence on
the represented units. These are derived from the same empirical evidence.
We retain the observed effect in every computational challenge so that
favorable recombinations cannot erase an observed failure. In this mandatory
role, it is the challenge's \emph{observed anchor}.

\subsection{The conjunctive decision rule}

A replacement policy must state how large an advantage it requires and how
broadly that advantage must survive the declared challenges. A familiar way to fix such thresholds would minimize an expected loss: assign a
...
Let $\delta\geq0$ be the required magnitude of advantage, $d\geq0$ the
advantage floor that a challenge must clear to count as surviving, and
$\gamma\in(0,1)$ the required fraction of surviving challenges. These are
policy commitments,
fixed before the effects are examined, rather than confidence levels or
quantities learned from those effects.

At each stage, let $\theta$ denote the retained magnitude and $\psi_d$ the
fraction of challenges that clear $d$, as defined for that stage below.
The stage passes only if
\begin{equation}
\theta>\delta
\qquad\text{and}\qquad
\psi_d>\gamma.
\label{eq:context-decision-rule}
\end{equation}
Both conditions must hold. The observed gate and the anchored computational
assessment apply this same conjunctive policy to different collections of
challenges.

\subsection{The observed gate}

At the observed gate, $\theta=z^{\mathrm{obs}}$ and
$\psi_d=\mathbf1\{z^{\mathrm{obs}}>d\}$. The rule therefore requires
\begin{equation}
z^{\mathrm{obs}}>\delta
\qquad\text{and}\qquad
\mathbf1\{z^{\mathrm{obs}}>d\}>\gamma.
\label{eq:context-observed-gate}
\end{equation}
With one observed evaluation, survival is $1$ if the effect clears $d$ and
$0$ otherwise. For any $\gamma\in(0,1)$, the survival requirement therefore
reduces to $z^{\mathrm{obs}}>d$, and the conjunction is equivalent to
$z^{\mathrm{obs}}>\max(\delta,d)$. Thus $\gamma$ remains in the rule, but
varying it within $(0,1)$ cannot change this binary gate's verdict. When
$\delta=d=0$, the gate requires only a strictly positive observed effect.
Failure of either condition returns no verdict for $A:B$. It does not establish the
reverse claim. Because the observed anchor remains mandatory, computation
cannot reverse this failure.

\subsection{The anchored computational challenge}

If the gate passes and a computational procedure belongs to the declared
assessment, select $K_C$ distinct computational effects, where $1\leq K_C\leq J$.
Write $\mathcal I_{J,K_C}$ for the collection of all such index subsets. Each
complete challenge returns
\[
\min\left\{z^{\mathrm{obs}},\min_{j\in I}z_j^{\mathrm{comp}}\right\},
\qquad I\in\mathcal I_{J,K_C}.
\]
The weakest effect controls each challenge. Average only after those minima
have been formed to obtain the observed-anchored supported magnitude:
\begin{equation}
\Thetaanc_{A:B;K_C}
=\binom{J}{K_C}^{-1}
\sum_{I\in\mathcal I_{J,K_C}}
\min\left\{z^{A:B,\mathrm{obs}},
\min_{j\in I}z_j^{A:B,\mathrm{comp}}\right\}.
\label{eq:context-supported-magnitude}
\end{equation}
The average uses the uniform counting
measure over a finite collection of declared challenges. It is not an
expectation over unseen empirical evaluations. Noncompensation operates
inside each complete challenge; the final average can tolerate some failed
challenges, subject to the separate survival requirement.

Let $R_s=\sum_{j=1}^J\mathbf1\{z_j^{\mathrm{comp}}>s\}$. Literal survival
above level $s$ is the fraction of complete challenges whose anchor and every
selected computational effect clear that level:
\begin{equation}
\Psianc_{K_C,s}
=\mathbf1\{z^{\mathrm{obs}}>s\}
\frac{\binom{R_s}{K_C}}{\binom{J}{K_C}}.
\label{eq:context-literal-survival}
\end{equation}
An anchor at or below the floor makes survival zero. For example, with an
anchor above zero and computational effects $(0.10,0.20,-0.05)$, exactly one
of the three order-two subsets clears zero, giving survival $1/3$.

The full context-specific rule supports replacement only when
\begin{equation}
\Thetaanc_{A:B;K_C}>\delta
\qquad\text{and}\qquad
\Psianc_{A:B;K_C,d}>\gamma.
\label{eq:context-full-rule}
\end{equation}
This is the same two-part rule, but computational survival can now be
fractional. The magnitude threshold $\delta$ and the breadth requirement
$\gamma$ therefore impose distinct conditions, both of which must pass.
...
favorable challenge minima from compensating for too many failures at $d$.
Passing the observed gate but failing the full rule means that the observed
advantage did not survive the declared recombinations of its represented units.

The thresholds and computational order play three distinct roles. The floor $d$
...
could go wrong, hence a claim that is easier to refute. The order $K_C$ fixes the \emph{stringency} of aggregation: it controls how a single
...
required \emph{breadth}: how many of the declared challenges must show clean
survival. The joint empirical alternative in Appendix~\ref{app:joint-empirical}
additionally specifies empirical order $K_E$, which determines how failures
across its admitted evaluations compound.
...
$\gamma$ & a probability or confidence of success & the fraction of declared challenges that must show clean survival \\
$K_C$ & a sample size or count of replications & how failures compound within a single challenge \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{Properties and reduced assessment}
\label{sec:context-properties}

Magnitude and survival describe the same completed challenges. If all effects
lie in $[a,b]$, averaging
$t=a+\int_a^b\mathbf1\{t>s\}\,ds$ over their minima gives
\[
\Thetaanc_{A:B;K_C}
=a+\int_a^b\Psianc_{A:B;K_C,s}\,ds.
\]
Thus magnitude integrates the survival curve, while literal survival evaluates
it at the policy floor. Anchoring both quantities preserves this connection.

Every challenge includes the observed effect, so
\[
\Thetaanc_{A:B;K_C}\leq z^{A:B,\mathrm{obs}},
\qquad
\Psianc_{A:B;K_C,d}\leq\mathbf1\{z^{A:B,\mathrm{obs}}>d\}.
\]
These inequalities justify the observed gate as an exact stopping rule.
Enlarging the condition set or increasing $K_C$ cannot increase either
supported magnitude or literal survival at a fixed floor. The proofs follow
by comparing the minima of nested selections; Appendix~\ref{app:support-proofs}
also establishes the more general identities used by the alternative empirical
assessment.

If no scientifically defensible computational procedure is available, or its
omission was prespecified for a practical reason, the analysis may stop at the
observed gate. A passing verdict then applies to the reduced observed-only
assessment. Formally this is $K_C=0$, with an empty computational minimum of
$+\infty$. A computational stage that was declared and failed cannot
subsequently be omitted to report a passing full verdict. A smaller positive
$J$ is a noisier approximation to the same declared challenge, not a weaker
substantive assessment.

The reverse comparison negates the paired profile before taking the condition
infimum and all computational minima. Its retained effect is therefore not
generally the negative of the forward retained effect. With matched
conditions, draws, and selections,
\[
\Thetaanc_{A:B;K_C}+\Thetaanc_{B:A;K_C}\leq0.
\]
Both directions can fail when different conditions or computational
evaluations defeat them. Neither failure establishes equivalence.
...
The construction preserves a difference that no algebra should erase. Empirical
...
particular represented units without changing the empirical scope. Shared
units or coordinated perturbations require a computational design that
preserves the relevant dependence. The graph reductions do not require
independence between empirical contexts or multiply their survival fractions.

\subsection{The observed gate is an exact stopping rule}

Calculate the observed retained effect before generating computational
replications for the primary verdict. If it fails either requirement in
Equation~\eqref{eq:context-observed-gate}, anchor domination proves that the
full anchored rule also fails. The primary analysis can stop with no verdict.
If the observed gate passes, generate the computational effects and evaluate
the full anchored quantities.
...
If one computational retained effect in evaluation $D$ exceeds $s$ with probability
$p(s)$ under that law, then the idealized iid order-$K_C$ survival is
\begin{equation}
q^{\mathrm{anc},\infty}_{K_C,s}
=
\mathbf 1\{z^{\mathrm{obs}}>s\}p(s)^{K_C}.
\label{eq:context-ideal-survival}
\end{equation}
The finite distinct-subset fraction
\[
\binom{R_s}{K_C}/\binom{J}{K_C}
\]
is a U-statistic approximation to that target. This distinction explains
why $K_C$ is substantive while $J$ is numerical. Lowering $K_C$ weakens the
declared challenge; lowering $J$ makes the same target less accurately
approximated.
...
checked separately from Monte Carlo stability. Appendix~\ref{app:context-computation}
...
The context-specific decision rule answers an oriented question
...
receive support against one rival and fail against another, and contextual comparisons may disagree. The pairwise results can therefore justify
...
When this admissible set contains several systems but practical use requires
one, a subsequent operational selection ladder may make that choice. Every
stage of that ladder must remain visibly separate from the evidential rule
that produced the set.
...
determine which vertices survive the evidential reduction. A separately
labeled operational ladder may subsequently use retained numerical information
to choose among those survivors.


\subsection{The maximal set within a context}

Within a context, a model remains eligible when no rival has established a
supported claim to replace it. This gives a nonempty set under the current
assessment because the observed gate prevents cycles of replacement claims.
A supported CNAP direction has a strictly positive observed paired advantage
at every declared prevalence, so its successor has higher CNAP at any one
fixed prevalence in that set. The successor in a supported AUROC direction
likewise has higher empirical AUROC at the uniform case mix. Following
replacement arrows therefore strictly decreases a scalar score, which cannot
return to its starting value in a cycle.

A more general graph definition also accommodates policies that permit cycles.
A strongly connected component is a set of vertices mutually reachable by
directed paths. Contracting these components gives an acyclic graph. A source
component has no incoming edge from another component. Write
$\operatorname{srcSCC}(G_e)$ for the union of all source components; this is
the context's \emph{graph-maximal set}. A finite nonempty graph always has at
least one source component. Under the current assessment each component is a
singleton, so the definition returns precisely the models with no incoming
supported loss. Appendix~\ref{app:multi-system} develops the general
construction and its relation to social choice.

\subsection{Two cross-context claims}
...
The scientific interpretation depends on the claim being
...
CNAP- and AUROC-based replacement claims define separate graphs. The
operational construction below is applied within one such analysis; its final
tie-break uses the declared metric-specific regret.

For the acyclic graphs supplied by the current assessment,
candidate-conservative survivors are contained in replacement-conservative
survivors. Appendix~\ref{app:multi-system} explains why this inclusion need
not hold for general cyclic graphs.

The context regime determines the scientific meaning of these conclusions.
Neither graph rule weights a context by its observation count or treats
computational draws as additional contexts. Neither requires statistical
independence among the admitted evaluations. A supported exclusion and a
missing replacement direction remain context-specific evidential results.
Changing the roster changes the challengers and requires a new reduction;
exclusion from eligibility alone does not erase a model's relevance to other
pairwise claims on the declared roster.

Write $S_0$ for the survivor set returned by the declared cross-context rule.
Figure~\ref{fig:assessment-workflow} summarizes the construction from the
within-context assessments to this set.

\begin{figure}[!t]
\centering
\includegraphics[width=\textwidth]{manuscript_images/context-replacement-workflow.pdf}
\caption{Summary of context-specific assessment and cross-context graph reduction.
Within each context, the paired condition challenge supplies an observed gate.
Passing claims face an anchored computational challenge when declared. The
resulting replacement directions define one graph per context; the chosen
graph reduction determines admissibility across contexts.}
\label{fig:assessment-workflow}
\end{figure}

\section{Operational choice among survivors}
\label{sec:operational}

An empty $S_0$ provides no eligible choice; a singleton needs no further
comparison. If practical use requires choosing among several survivors, first
compare the strength of the remaining criticism against each one, then use
performance deficits to resolve any remaining ties. The evidential result
$S_0$ remains fixed throughout both stages.

First compare each survivor's vulnerability to incoming claims from every
other survivor in every context. With all commitments except $\gamma$ fixed,
the activation strength of a direction is the supremum of the survival
requirements under which it would be supported. It is zero if the magnitude
requirements fail. Sort each candidate's incoming strengths from greatest to
least and minimize these vectors lexicographically: compare the strongest
threat first, and proceed to the next coordinate only on a tie. This returns
the least-vulnerable set $T$ without letting several weak threats offset one
strong threat.

If $T$ remains plural, compare performance deficits against every member of
the frozen $S_0$. For CNAP, each evaluation's regret is the largest paired
deficit to an admissible comparator anywhere in the declared operational
prevalence range. For AUROC, the default regret uses the observed case mix.
Sort these evaluation-level regrets in descending order and minimize them
lexicographically over $T$. Call the resulting operational set
$S_{\mathrm{op}}$. Identical complete vectors remain tied. A system outside
$T$ remains an admissible benchmark if it belongs to $S_0$. The sequence is
\[
S_0
\ \longrightarrow\ 
T\quad\text{(incoming replacement strength)}
\ \longrightarrow\ 
S_{\mathrm{op}}\quad\text{(metric-specific regret)}.
\]

This priority is substantive. Regret resolves only ties left by the incoming
comparison; it cannot overturn a unique member of $T$ or guarantee that
member's absolute performance. Operational selection also cannot create an
admissible model when $S_0$ is empty. Appendix
\ref{app:multi-system-operational-criticism} gives the definitions and a worked
example. The operational choice and fixed-policy survivor set are reported
separately.

Membership in $S_0$ does not certify causal appropriateness, and zero regret
does not establish adequate absolute performance. Shortcomings shared by all
survivors may remain invisible to this relative comparison. Changing the
roster or the declared evaluations requires recomputing the graph reduction
and operational result.

\section{Empirical illustration: Bank Marketing}
\label{sec:illustrations}

One temporal empirical evaluation illustrates the context-specific assessment.
A deliberately strong comparison passes the observed gate and proceeds to the
anchored computational challenge; a close comparison fails at the gate. The
policy uses $\PSet=[0.01,0.50]$, $\delta=d=0$, $\gamma=0.81$, and
computational order $K_C=2$. These choices define this illustration and are
not proposed as defaults. Section~\ref{sec:proteingym-contexts} then applies
cross-context graph reduction to a collection of empirical evaluations.

\subsection{Evaluation and paired comparisons}
...
The temporal holdout supplies one empirical evaluation. Random forest minus full
...
\section{Empirical illustration: ProteinGym}
\label{sec:proteingym-contexts}

ProteinGym organizes its deep-mutational-scanning benchmark into
assay-indexed datasets containing experimentally measured variant effects
and released prediction scores \cite{notin2023proteingym}. Each retained
assay supplies one empirical context for a complete directed comparison of
three models. The question is which models remain admissible when these
context-specific assessments are combined by the candidate-conservative rule.

\subsection{Evaluations and assessment policy}

We compared released zero-shot scores from the EVE ensemble evolutionary
model, the ESM-1v ensemble protein language model, and the ESM-2 650M protein
language model \cite{frazer2021eve,meier2021esm1v,lin2023esm2}. No model was
fit or tuned for this study. EVE is itself an alignment-based deep generative
evolutionary model.
...
Released fitness scores were negated so that larger analyzed values represented
stronger predicted deleteriousness. The aligned cohort contained 277,873
substitutions in 91 assays, representing 80 proteins and 60 publications.
Substitutions are paired observations within an evaluation, not separate
empirical replications. Assays receive no observation-count weights, and
retaining their shared proteins and publications does not assert independent
scientific replication.

Both metric assessments use $\delta=d=0$, $\gamma=0.81$, and $K_C=2$.
For CNAP, the observed anchor is the smallest paired advantage over
$\PSet=[0.01,0.90]$. Ordinary AUROC assigns half credit to score ties and
evaluates the observed within-class composition. Its prevalence invariance
leaves no corresponding prevalence challenge; this illustration applies no
additional within-class concentration challenge. It is the $\Gamma=1$
boundary of the concentration construction in Appendix~\ref{app:auroc}.

Both orientations of every model pair are assessed separately in each assay.
A direction that passes its observed gate receives $J=200$ computational
evaluations, generated by sampling residue positions with replacement and
carrying all retained substitutions at each selected position. Both supported
magnitude and literal survival must then pass the observed-anchored rule.
The two metric assessments use the same position draws, model roster, and
decision thresholds, but their condition challenges differ. Each produces
its own context graphs and survivor set; the metrics are not aggregated.
These choices define a retrospective illustration, not a prospectively
validated deployment policy. Appendix~\ref{app:proteingym-context-checks}
gives the resampling procedure and numerical checks.

\subsection{Supported replacements and cross-context survivors}

Under each metric, the candidate-conservative set is empty: every model has
an incoming supported replacement claim in at least one assay. In contrast,
replacement-conservative reduction retains all three models because no
directed replacement claim is supported in all 91 assays. These are different
answers to the two questions in Section~\ref{sec:roster}, not conflicting
verdicts about the same claim.

Of 546 directed assessments per metric, 222 CNAP directions and 273 AUROC
directions pass the observed gate. The computational challenge retains 125
and 181 supported directions, respectively. Table~\ref{tab:proteingym-context-results}
shows their distribution across model pairs. The counts describe
context-specific replacement claims; they neither rank the models by votes
nor determine admissibility by a majority rule.

\begin{table}[htbp]
\centering
\caption{Numbers of assays supporting each replacement after the full
single-context assessment. An arrow points from the supported successor to
the incumbent it may replace. Each direction is assessed in all 91 assays,
using CNAP over 1\%--90\% prevalence or ordinary AUROC.}
\label{tab:proteingym-context-results}
\begin{tabular}{@{}lrr@{}}
\toprule
Supported replacement & CNAP & AUROC \\
\midrule
ESM-1v ensemble $\to$ EVE ensemble & 25 & 35 \\
EVE ensemble $\to$ ESM-1v ensemble & 17 & 25 \\
ESM-2 650M $\to$ EVE ensemble & 31 & 36 \\
EVE ensemble $\to$ ESM-2 650M & 13 & 24 \\
ESM-2 650M $\to$ ESM-1v ensemble & 25 & 35 \\
ESM-1v ensemble $\to$ ESM-2 650M & 14 & 26 \\
\bottomrule
\end{tabular}
\end{table}

Neither protein language model supports replacing EVE throughout the declared
assay collection under either metric. Both nevertheless support replacement
in particular assays, while other assays support the reverse direction.
Local improvements therefore do not identify a model that remains admissible
everywhere. This does not establish equivalence or inadequate performance in
every context. Position resampling probes dependence on represented observations;
it adds no empirical evidence about unseen proteins, assays, or measurement
mechanisms.
...
Within each admitted context, the observed retained effect records what remains
after the condition challenge. Anchored computational support records what
remains after that effect faces the declared computational challenge. Both are
indexed by the orientation, condition set, and empirical regime; the full
assessment additionally specifies the computational procedure and order.
Cross-context admissibility is indexed by the declared graph reduction.
...
decision, with each computational challenge retaining its weakest effect and each
context retaining its own verdict. Candidate-conservative reduction allows
each context to exclude a candidate; replacement-conservative reduction instead
requires agreement on a replacement direction. Literal survival within a
context is the exact fraction of its declared anchored challenges that clear
the floor, not a confidence level. The resulting verdict
...
The ordered finite-list form of $-S_K$ in Appendix~\ref{app:joint-empirical}
resembles a spectral loss functional
...
\cite{nadeau2003inference,bouckaert2004estimating,demsar2006statistical}. The graph reductions here operate on context-specific replacement claims.
The alternative empirical subset average in Appendix~\ref{app:joint-empirical}
is an exact finite selection over admitted evaluations. Neither construction
supplies a sampling law for unseen datasets.
...
assumed law; the context-specific survival fraction is a census under the declared uniform
measure over the $\binom{J}{K_C}$ anchored computational selections. The
alternative empirical assessment similarly counts subsets of actual anchors
and, when declared, their computational effects. Nothing unrealized enters, so the two are different objects despite
...
A report should identify each context-specific claim, the stage at which it
survived or failed, and the rule that combined the resulting graphs.

\begin{table}[!t]
\centering
\caption{Minimum reporting content for context-specific assessment and graph reduction.}
\label{tab:reporting}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}X@{}}
\toprule
Component & Report \\
\midrule
Claim and policy & Model roster; candidate--incumbent orientation; context-specific $\delta,d,\gamma$; reverse assessment; full or reduced scope \\
...
Empirical evidence & $\Regime$; context admission and units; known dependence or multiplicity; pooling rule; $N_e$; observed anchors and gate results \\
Computational challenge & $\Boot$ and resampling unit; pairing and dependence preservation; $J_e$ and $K_C$; anchored support and survival \\
Diagnostics & Binding anchors; computational effects below their anchors; values near $d$; seed and $J_e$ stability; sensitivity to prespecified alternative orders, conditions, and floors \\
Graph reduction & Supported directions and valid no-verdicts; unresolved computation; primary cross-context rule; context-specific maximal sets and joint $S_0$ \\
Operational choice & Each invoked stage; frozen challenger and benchmark set; incoming-strength vectors; metric-specific regrets and their binding contexts; remaining ties \\
Scope & Represented case types and contexts; absent mechanisms; construction history; whether a reference randomization is asserted; external premise required for deployment \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{quote}
In context [name], the observed retained effect for $A:B$ was [value], so the
claim [passed/failed] the observed gate. [If passed:] With $J$ paired
computational evaluations and order $K_C$, anchored supported magnitude was
[value] and literal survival above $d$ was [value]. The assessment returned
[supported replacement/no verdict] under the declared thresholds. Across the
admitted contexts, [candidate-conservative/replacement-conservative] reduction
retained $S_0=[\ldots]$. [If invoked:] Operational refinement selected
[system or tied set] while preserving $S_0$ as the evidential conclusion.
\end{quote}
An empty candidate-conservative set should be reported as such. If the
observed gate fails, anchor domination makes computation unnecessary for that
direction. A passing observed-only assessment must be identified as reduced
when no computational challenge was declared.

\subsection{Beyond AP}

The observed-anchored assessment applies to any metric that supplies condition-challenged observed and computational effects. Because AUROC is invariant to target prevalence, Appendix~\ref{app:auroc} instead challenges capped within-class redistributions of represented cases and applies the same context-specific replacement rule.
...
within that empirical evaluation. A staged computational extension can
...
Across contexts, the supported comparisons define separate graphs.
Candidate-conservative reduction requires a system to remain admissible in
every context. Replacement-conservative reduction instead retains only
directions supported throughout the declared scope before finding admissible
systems. The rule is chosen according to the scientific claim. Operational
selection, when needed, follows this evidential reduction.
...
Within a context, failure identifies where the available evidence ceased to support the oriented
...
\section{Criticism-first operational selection}
\label{app:multi-system-operational-criticism}

The graph reductions of Section~\ref{sec:roster} answer which systems remain scientifically
admissible under a fixed policy. They need not return one system. If practical
use requires a unique choice, the remaining question is operational: how
should one member of a plural admissible set be selected without rewriting the
policy that produced it? The complete sequence is
\[
\begin{gathered}
\text{full-roster fixed-policy graph reduction}
\longrightarrow S_0\\
\longrightarrow
\text{lexicographic incoming-survival comparison within }S_0
\longrightarrow T\\
\longrightarrow
\text{lexicographic metric-specific regret against }S_0
\longrightarrow S_{\mathrm{op}}.
\end{gathered}
\]
The complete roster $V$ determines the fixed-policy graphs and hence the
evidential survivor set $S_0$. Once obtained, $S_0$ is frozen throughout the
operational stages. Only its members are eligible replacement alternatives:
they supply the incoming challengers in the first operational stage and the
performance benchmarks in the second. A system that does not enter $T$
remains in $S_0$ and retains both roles. If $S_0$ is empty, there is no
eligible choice. If it contains one system, that system is selected without an
operational comparison.

\paragraph{Running example: fixed-policy reduction.}
Take the complete roster $V=\{A,B,C,D\}$ in two declared contexts. After all
planned pairwise assessments, suppose each context's fixed-policy graph at
$\gamma_0=0.81$ contains the edges
\[
A\longrightarrow D,
\qquad
B\longrightarrow D,
...
C\longrightarrow D,
\]
and no others. The singleton components $\{A\}$, $\{B\}$, and $\{C\}$ have no
incoming edges, while $\{D\}$ does. Either cross-context reduction therefore
gives $S_0=\{A,B,C\}$. System $D$ cannot participate in the operational ordering,
although its pairwise results remain in the evidential record.

The two operational stages have a substantive priority. The first asks which
admissible systems are least vulnerable to incoming replacement claims in the
finite evidence. Only if that criticism-first comparison remains set-valued
does the second ask which tied system has the smallest worst performance
deficit under the metric-specific operational rule: a declared target-prevalence
range for CNAP, or the observed case mix for the default AUROC rule.

\paragraph{Incoming-survival comparison.}
Literal survival and its requirement play different roles. The quantity
$\Psi$ measures the breadth of a directional claim in the finite evidence,
whereas $\gamma$ is the declared cutoff that turns that measurement into a
binary verdict. For example, two directions with survival $0.80$ and $0.20$
both fail when $\gamma=0.81$, even though the former is closer to producing an
edge. The fixed-policy graph intentionally suppresses that difference. An
operational reduction can retain it without claiming that the policy threshold
was chosen after the result.

Fix the metric $m$, graph-level context $e$, survival floor, magnitude
threshold, computational order, condition set, computational procedure, and every
other element of the assessment except $\gamma$. For the direction $A:B$, let
$\psi^{\mathrm{obs}}_{m,e}(A:B)$ be observed literal survival and, when the
computational level is declared, let
$\Psi^{\mathrm{anc}}_{m,e}(A:B)$ be full anchored literal survival. Define the
\emph{edge activation strength}
\begin{equation}
w_{m,e}(A\to B)
=
\begin{cases}
\min\!\left\{
\psi^{\mathrm{obs}}_{m,e}(A:B),
\Psi^{\mathrm{anc}}_{m,e}(A:B)
\right\},
& \text{if both magnitude requirements pass},\\
0, & \text{otherwise}.
\end{cases}
\label{eq:edge-activation-strength}
\end{equation}
For a reduced observed-only assessment, omit the full quantity and require
only the observed magnitude condition. Thus $w$ is the activation breakpoint:
the direction is supported exactly when $\gamma<w$, with all other
commitments held fixed. Equivalently, $w$ is the supremum of the breadth
requirements that would support the direction, with zero assigned when none
does. In a context with observed anchor above $d$, $J$ computational effects, and
$K_C=2$, it is
\[
w_{m,e}(A\to B)
=
\frac{\binom{R_{m,e,d}}{2}}{\binom{J}{2}},
\]
where $R_{m,e,d}$ counts computational effects above $d$, provided that the
observed and full supported magnitudes exceed $\delta$.

For eligible system $B\in S_0$, its strongest incoming activation strength is
\begin{equation}
\rho_m(B)
=
\max_{\substack{e\\A\in S_0\setminus\{B\}}}
w_{m,e}(A\to B).
\label{eq:incoming-survival-minimax}
\end{equation}
Smaller is better. The system minimizing $\rho_m$ requires the greatest
relaxation of breadth before its strongest admissible challenger can support a
replacement direction. This is a minimax rule: one strong incoming challenge
controls, so favorable comparisons cannot compensate for it.

The strongest incoming value is only the first coordinate of the complete
comparison. For each $B\in S_0$, collect all incoming strengths from admissible
alternatives in all declared graph-level contexts and sort them from largest to
smallest:
\begin{equation}
\mathbf w_m^{\downarrow}(B)
=
\operatorname{sort}_{\mathrm{descending}}
\left(
\bigl(w_{m,e}(A\to B)\bigr)_{
\substack{e\\A\in S_0\setminus\{B\}}}
\right).
\label{eq:incoming-survival-vector}
\end{equation}
First minimize the largest coordinate, then the second largest among systems
tied on the first, and continue through the complete vector. The
\emph{lexicographic minimax incoming-survival rule} returns
\begin{equation}
T
=
\operatorname*{arg\,min}^{\mathrm{lex}}_{B\in S_0}
\mathbf w_m^{\downarrow}(B).
\label{eq:incoming-survival-selection}
\end{equation}
It uses every admissible incoming challenge but never lets several weak threats
compensate for one strong threat. Identical complete vectors remain tied.
Systems in $S_0\setminus T$ are not eligible for final selection, but they
remain admissible challengers in the complete incoming-strength comparisons
and remain performance benchmarks in the tertiary comparison. The set $S_0$
is not recomputed after $T$ is obtained.

\paragraph{Running example: incoming-survival reduction.}
Continue with $S_0=\{A,B,C\}$ and suppose there are two declared graph-level
contexts. Each system then has four incoming activation strengths: two
admissible challengers in each of the two contexts. Suppose their descending
vectors are
\[
\begin{aligned}
\mathbf w_m^{\downarrow}(A)&=(0.76,0.61,0.45,0.20),\\
\mathbf w_m^{\downarrow}(B)&=(0.76,0.61,0.45,0.20),\\
\mathbf w_m^{\downarrow}(C)&=(0.79,0.50,0.30,0.10).
\end{aligned}
\]
The first coordinates exclude $C$ from the lexicographic minimum, while the
complete vectors for $A$ and $B$ coincide. Hence $T=\{A,B\}$. System $C$ can
no longer be selected, but it remains an admissible challenger and performance
benchmark because the operational analysis keeps $S_0$ fixed. System $D$ does
not regain either role.

\paragraph{Worst-target CNAP regret as a tertiary operational rule.}
For a CNAP-primary tournament, if $T$ contains more than one system, declare a nonempty compact operational
prevalence set $\Pi_{\mathrm{op}}\subset(0,1)$ before examining the operational
selection results. It may equal the assessment set $\PSet$; if it differs, its
relationship to the assessed scope and its substantive basis should be stated
explicitly. The realized prevalence in an evaluation receives no automatic
priority in this rule.

For empirical evaluation $D_e$, retain the outcome-blind tie convention of
Equation~\eqref{eq:tie-average} and define
\begin{equation}
q_e(B;\pi)
=
\CNAPta_{B,\pi}(D_e).
\label{eq:operational-cnap-profile}
\end{equation}
At a fixed prevalence, compare each remaining candidate with every admissible
alternative on the same units and at the same prevalence. Then challenge that
paired deficit over the complete operational prevalence set. For $B\in T$,
define
\begin{equation}
r_e^{\mathrm{range}}(B)
=
\sup_{\pi\in\Pi_{\mathrm{op}}}
\max_{C\in S_0}
\left\{
q_e(C;\pi)-q_e(B;\pi)
\right\}.
\label{eq:range-cnap-regret}
\end{equation}
This is $B$'s largest performance deficit to any admissible alternative
anywhere in the declared operational prevalence range within evaluation $e$.
Because $B\in S_0$, including $C=B$ makes the regret nonnegative. Different
comparators and prevalences may determine the regret in different evaluations.
A comparator in $S_0\setminus T$ remains eligible to determine the benchmark.

For each $B\in T$, sort the evaluation-level range regrets from largest to
smallest:
\begin{equation}
\mathbf r_{\mathrm{range}}^{\downarrow}(B)
=
\operatorname{sort}_{\mathrm{descending}}
\left(
r_1^{\mathrm{range}}(B),\ldots,r_M^{\mathrm{range}}(B)
\right).
\label{eq:range-cnap-regret-vector}
\end{equation}
The tertiary operational set is
\begin{equation}
S_{\mathrm{op}}
=
\operatorname*{arg\,min}^{\mathrm{lex}}_{B\in T}
\mathbf r_{\mathrm{range}}^{\downarrow}(B).
\label{eq:range-cnap-selection}
\end{equation}
The first coordinate minimizes the worst evaluation-level range regret; later
coordinates resolve ties in order. There is no averaging across prevalences or
evaluations, and identical complete vectors remain tied.

\paragraph{Empirical AUROC regret as a tertiary operational rule.}
For an AUROC-primary tournament, use AUROC regret at the default operational
concentration allowance $\Gamma_{\mathrm{op}}=1$. In the within-class weight
sets of Appendix~\ref{app:auroc}, this allowance fixes uniform weights within
each class. Let $a_e(B)$ be the empirical AUROC of system $B$ on all observed
units in evaluation $e$, with half credit for tied positive--negative pairs.
For $B\in T$, define
\begin{equation}
r_e^{\mathrm{AUROC}}(B;1)
=\max_{C\in S_0}\{a_e(C)-a_e(B)\}.
\label{eq:operational-auroc-regret}
\end{equation}
Including $C=B$ makes regret nonnegative. As in the CNAP rule, the benchmark
set remains all of $S_0$, including systems outside $T$. Sort the
evaluation-level regrets in descending order and minimize the complete vectors
lexicographically over $T$, retaining identical-vector ties. With one context,
this selects the largest empirical AUROC among the members of $T$. It does
not override either the fixed-policy reduction or the incoming-survival rule.

In the single-context setting with a nonnegative magnitude threshold and an
observed gate requiring a strictly positive paired AUROC gain, every empirical
AUROC maximizer on the complete roster has no incoming activation of positive
strength. Such maximizers survive in $S_0$ and have all-zero incoming profiles,
so they also belong to $T$. The default tertiary rule therefore returns the
complete roster's empirical AUROC maximizers in this setting. The severe tests
still distinguish supported replacement claims and evidential survivors, but
this operational rule does not select a lower empirical AUROC.

This operational allowance is distinct from the directional supported-replacement
breakdown factor $\Gamma^{\dagger}$: the latter describes the failure of an
initially supported claim as case-mix stress increases. The default tertiary
rule adds no case-mix stress or new computational evaluations. It uses all
observed data to resolve only a remaining operational tie. Choosing
$\Gamma_{\mathrm{op}}>1$ would instead require an explicitly declared
worst-case paired AUROC-regret optimization over the permitted within-class
redistributions; it is not part of this default rule.

\paragraph{Running example: worst-target CNAP regret.}
The incoming-survival comparison left $T=\{A,B\}$, so the tertiary rule is
invoked. Let $\Pi_{\mathrm{op}}=[0.01,0.50]$ and suppose two empirical
evaluations give the descending range-regret vectors
\[
\mathbf r_{\mathrm{range}}^{\downarrow}(A)=(0.04,0.01),
\qquad
\mathbf r_{\mathrm{range}}^{\downarrow}(B)=(0.02,0.015).
\]
The first coordinates select $B$, so $S_{\mathrm{op}}=\{B\}$. The binding
$0.02$ regret may, for example, arise in $D_1$ because $C$ exceeds $B$ most at
$\pi=0.05$. This is permitted even though $C\notin T$, because $C$ remains in
the frozen benchmark set $S_0$. The complete example has therefore reduced the
sets in the declared order:
\[
\{A,B,C,D\}
\longrightarrow
\{A,B,C\}
...
The priority between the two operational stages is substantive. If $T$ is a
singleton, the metric-specific tertiary rule is not invoked. Its selected member is
not thereby guaranteed to have small regret. Tertiary regret resolves
only ties left by the incoming-survival comparison; it neither overrides that
comparison nor imposes a general bound on the performance sacrificed by a
uniquely selected member of $T$.

\begin{table}[H]
\centering
\caption{The criticism-first multi-system selection ladder. Later operational
layers preserve the earlier fixed-policy result as a separately reported
conclusion.}
\label{tab:operational-selection-ladder}
\small
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.19\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
\toprule
Layer & Selectable systems & Challengers or benchmarks & Output \\
\midrule
Fixed-policy graph reduction & Complete roster $V$ & Complete roster $V$ & Evidential survivor set $S_0$ \\
Incoming-survival reduction & $S_0$ & Admissible alternatives $S_0\setminus\{B\}$ & Least-vulnerable set $T$ \\
Metric-specific regret reduction & Tied set $T$ & Fixed admissible set $S_0$; CNAP over $\Pi_{\mathrm{op}}$, or AUROC at $\Gamma_{\mathrm{op}}=1$ & Operational choice $S_{\mathrm{op}}$ or remaining tie \\
\bottomrule
\end{tabularx}
\end{table}

A minimum report preserves the fixed-policy result and every operational layer
invoked:
\begin{quote}
At the declared requirement $\gamma_0$, the primary graph reduction retained
$S_0$ as the evidentially admissible set. Holding that set fixed, the
lexicographic minimax incoming-survival comparison among admissible systems
retained [system or tied set $T$]. Its binding incoming claim had activation
strength [value] and arose from [admissible challenger and context]. [If $T$ is
plural:] Under the declared metric-specific tertiary rule,
system [name] had the lexicographically smallest descending regret
vector. Its largest regret was [value] in [evaluation], relative to
[admissible comparator], at prevalence [value] for CNAP or at
$\Gamma_{\mathrm{op}}=1$ for empirical AUROC regret. The operational result was
$S_{\mathrm{op}}=[\ldots]$. This operational choice does not replace the
fixed-policy verdict, reinterpret activation strength as a policy requirement,
or give tertiary regret priority over the incoming-survival comparison.
\end{quote}

Changing the roster can change paths, components, and maximality, so a revised
roster requires a new graph reduction. It can also change $S_0$, the incoming
strength vectors, the tied set $T$, the admissible performance benchmarks, and
the operational choice. Changing the empirical evaluations or
$\Pi_{\mathrm{op}}$ can change the range-regret vectors and their binding
comparators or prevalences. Neither the multi-system graph reduction nor the
operational choice supplies a simultaneous error guarantee beyond that
provided by the prespecified evidential policy.

\section{Source components and general support graphs}
\label{app:multi-system}

The current context-specific observed gates provide acyclic graphs, as shown
in Section~\ref{sec:roster}. The source-component definition also handles
more general support policies. This appendix records that construction and
its structural relation to social choice; it does not assert cycles under
the current gate.

A general support policy may produce
\[
...
For the reference argument, the notation of Appendix~\ref{app:joint-empirical}
also covers one context by setting $M=K_E=1$.
...
\section{Computing a context-specific assessment}
\label{app:context-computation}

Compute the observed condition infimum first and stop if it fails
Equation~\eqref{eq:context-observed-gate}. If the gate passes, generate the
declared paired computational evaluations. Sort their retained effects as
$t_{(1)}\leq\cdots\leq t_{(J)}$. With observed anchor $u$, the exact finite
supported magnitude is
\[
\Thetaanc_{K_C}
=\binom{J}{K_C}^{-1}
\sum_{i=1}^{J-K_C+1}
\binom{J-i}{K_C-1}\min\{u,t_{(i)}\}.
\]
The coefficient counts the distinct subsets whose first indexed minimum is
$t_{(i)}$. Literal survival follows from the number of effects strictly above
the floor and the anchor indicator. No enumeration of all subsets is needed.
...
raise the retained effect. For each evaluation, a one-dimensional condition optimizer
...
For the computational collection, repeat the calculation under independent
seeds and increasing counts $J$. Compare anchored magnitude, literal survival
at $d$, the effects limited by the anchor, and the verdict. A stable
unanchored projection is insufficient if anchoring changes which effect levels
control the assessment. A computational standard error describes approximation under
the declared resampling law. Its use requires applicability to that design
and does not supply uncertainty about unseen empirical evaluations.

\subsection{ProteinGym single-context computation and checks}
\label{app:proteingym-context-checks}

The studies in Section~\ref{sec:proteingym-contexts} use the same aligned
scores, labels, and assay roster as the joint empirical illustration in
Section~\ref{sec:proteingym-illustration}, but assess each context separately.
There is no empirical subset order across assays. Within each assay, every
computational draw samples its observed number of distinct residue positions
with replacement and carries all retained substitutions at each selected
position. A draw lacking either outcome class is redrawn completely, with a
limit of 10,000 attempts. A fixed seed of 20260917, stable assay identifiers,
and canonical position ordering supply the same draws across models,
orientations, and metrics. All planned comparisons must return valid results
before graph reduction; a failed observed gate stops that direction.

CNAP effects are minimized on a 257-point prevalence grid with local
refinement, including explicit minimization inside both endpoint grid cells,
at tolerance $10^{-8}$. Every observed pair and eight evenly spaced
computational draws per evaluated pair are checked on a 513-point grid at
tolerance $10^{-9}$. Across 1,776 checked computational draws and all observed
pairs, the largest retained-effect change is below $8\times10^{-16}$, with no
observed-gate or computational-floor disagreement. The endpoint-cell search
corrects an omission in the earlier joint illustration's grid search, lowering
some observed anchors by at most $3.94\times10^{-5}$ without changing their
single-context observed-gate classifications. The earlier artifacts are
retained separately. These are numerical checks of a continuous condition
search, not global optimization certificates.

Ordinary AUROC uses integer concordance counts, with half credit for ties,
before division to preserve exact zero paired differences. All observed
pairs and 2,184 computational draws are checked independently using average
ranks after expanding sampled multiplicities. The largest difference is below
$2\times10^{-16}$; observed effects agree with the saved ordinary AUROC
values within $3\times10^{-16}$. No concentration search beyond $\Gamma=1$
is performed. For both metrics, every saved anchored magnitude and survival
fraction is independently recomputed from the finite computational list.

Comparing the first 100 draws with the full 200 changes eight supported
arrows for each metric and changes the admissible set in four CNAP contexts
and five AUROC contexts. Both cross-context survivor sets remain unchanged.
Because this comparison shares draws, it is not an independent-seed
replication and does not establish an exact verdict under the ideal
computational law. Neither these numerical checks nor the requirement
$\gamma=0.81$ supplies statistical confidence about unseen populations.
The saved plans, pairwise reports, and comparison tables are provided with
the code in \texttt{supported\_ap\_code/domain\_study/}, under
\texttt{proteingym\_contexts/} and \texttt{proteingym\_auroc\_contexts/}.
...
For empirical evaluation $D$, define its observed AUROC anchor at
concentration factor $\Gamma$ as
\[
z^{\mathrm{obs}}(\Gamma)
=Z_\Gamma^{A:B}(D).
\]
For computational evaluation $D^{(j)}$ generated under $\Boot$, define
\[
z_j^{\mathrm{comp}}(\Gamma)
=Z_\Gamma^{A:B}(D^{(j)}).
\]
The case-mix optimization is performed inside every full empirical or
computational evaluation. At each fixed $\Gamma$, these effects enter
Equations~\eqref{eq:context-supported-magnitude} and
\eqref{eq:context-literal-survival}, giving
$\Thetaanc_{A:B;K_C}(\Gamma)$ and
$\Psianc_{A:B;K_C,d}(\Gamma)$.
The concentration allowance is common to the anchor and computational draws,
while their adverse weights may differ. Both profiles are nonincreasing in
$\Gamma$. Across contexts, any allowance used to define graph edges belongs
to the declared policy; it is not chosen after comparing the results.
...
=
\inf\left\{\Gamma\geq1:
\Thetaanc_{A:B;K_C}(\Gamma)\leq\delta
\ \text{or}\ 
\Psianc_{A:B;K_C,d}(\Gamma)\leq\gamma
\right\}.
...
observed-only breakdown. The observed AUROC effect remains a mandatory anchor throughout both
challenges. The joint empirical alternative applies the same cellwise effects
to its nested operator, as described in Appendix~\ref{app:joint-empirical}.

\subsection{Bank Marketing AUROC companion}
\label{app:auroc-illustrations}

The Bank Marketing data used in Section~\ref{sec:illustrations} also
illustrate the AUROC construction. The calculations reuse the paired scores,
evaluation units, orientations, and replacement thresholds already declared
for that illustration. Numerical lower and upper bounds below are optimization
...
reported breakdown bracket. This context-specific assessment uses $K_C=2$ and $J=80$.}
...
required computational cell. The same computational evaluation $D^{(j)}$ is
...
\section{Joint replacement assessment across empirical evaluations}
\label{app:joint-empirical}

This appendix asks whether one oriented replacement claim survives a declared
challenge across several admitted empirical evaluations. It combines their
paired effects before forming a replacement verdict. The main construction
instead forms context-specific verdicts and combines their graphs. The two
procedures need not agree and are not interchangeable stages of one analysis.

For example, suppose $A$ supports replacement of $B$ in $D_1$, while $B$
supports replacement of $A$ in $D_2$. For the two-system roster,
candidate-conservative graph reduction returns an empty set;
replacement-conservative reduction retains both systems. A joint assessment
with empirical order $K_E=2$ also rejects both oriented claims because each
faces an adverse observed anchor. Its graph has no edges and retains both
systems. Agreement with the replacement-conservative result in this example
does not make the joint operator an edge-intersection rule: it uses numerical
effects and its own empirical--computational challenge.

\subsection{Scope and notation}

Let $D_1,\ldots,D_M$ be the empirical evaluations admitted by $\Regime$.
Each supplies an observed retained effect $z_m^{\mathrm{obs}}$ and, when
declared, $J_m$ computational effects $z_{mj}^{\mathrm{comp}}$ obtained by
the same paired condition challenge used in the main text. The additional
order $K_E$ states how many distinct empirical evaluations are jointly faced;
$K_C$ remains the number of computational effects selected within each one.
The assessment specification is
\[
\Assess=(\CSet,\Regime,\Boot,K_E,K_C).
\]
Increasing $M$ adds empirical evidence, whereas increasing $K_E$ changes the
challenge applied to a fixed collection. Increasing $J_m$ refines a
computational approximation, whereas increasing $K_C$ changes its target.
No choice of $J_m$ turns one empirical evaluation into several.

An empirical subset contributes its weakest effect before averaging. At
$K_E<M$, the declared aggregate can tolerate failed evaluations; it does not
give each context the exclusion authority of candidate-conservative graph
reduction. Context admission, dependence, and multiplicity remain scientific
commitments rather than assumptions of independent sampling from an unseen
population. Shared units or coordinated perturbations require a joint
computational law; the product formulas below apply only when row selections
are independent.

\subsection{Support across empirical and computational challenges}
\label{sec:hierarchy}
...
\subsubsection{The finite-list support operator}
...
\subsubsection{One complete nested challenge}
...
\subsubsection{Observed-anchored nested support}
...
\subsubsection{Why the hierarchy cannot be flattened or summarized by rows}
...
\subsubsection{Literal survival of a declared floor}
...
\subsubsection{Magnitude and survival describe the same challenges}
...
\subsubsection{Boundary cases and challenge properties}
...
\]
This is an \emph{observed-only joint assessment};
...
diagnostic, but it is not the joint assessment developed in this appendix.
...
\subsection{The joint staged decision}
\label{sec:decision}

Use the magnitude threshold $\delta$, floor $d$, and breadth requirement
$\gamma$ with the meanings given in Section~\ref{sec:context-rule}. Here
$K_E$ adds empirical stringency: at order one the observed magnitude is the
mean anchor, while at $K_E=M$ it is the weakest anchor. The pair
$(K_E,\gamma)$ determines how many observed failures the survival requirement
tolerates. These choices belong to the joint claim and do not alter the graph
rules in the main text.

\subsubsection{Stage 1: the observed empirical gate}
...
\subsubsection{Stage 2: the computational challenge}
...
\subsubsection{Reduced assessment and reverse orientation}
...
\subsection{Computational target}
...
\subsection{ProteinGym illustration of the joint empirical assessment}
...
This illustration uses $\PSet=[0.01,0.90]$, $\delta=d=0$,
$\gamma=0.81$, and empirical order $K_E=2$. All examined orientations fail
the observed gate, so computation is unnecessary for their verdicts. These
are results under the joint empirical assessment, not context-specific graph
reductions.
...
Because the observed assay anchors fail the necessary joint empirical gate, resampling
...
The joint AUROC companion below also reports the observed-only AUROC
boundary for these same 91 assays. Because every declared comparison fails
that joint empirical gate, no within-assay computational AUROC collection is
generated for this alternative assessment. The separate context-specific
assessments in Section~\ref{sec:proteingym-contexts} instead apply the observed
gate within each assay and compute every passing direction's challenge.


\subsection{ProteinGym joint empirical AUROC companion}
For this companion, each observed and computational cell would be
$Z_\Gamma^{A:B}(D)$ from Appendix~\ref{app:auroc}. At each common
$\Gamma$, the cells enter the joint nested operator, with evaluation-specific
adverse weights. If the joint rule passes at one, its breakdown is the first
$\Gamma$ at which joint magnitude or survival fails. The same monotonicity
and numerical certification requirements apply.
...
\subsection{Computation for several empirical evaluations}
\label{app:computation}

\subsubsection{Observed gate}
...
\subsubsection{Finite-array sweep}
...
\subsubsection{A deliberately favorable ProteinGym stress test}
\label{app:proteingym-nested}

The joint empirical ProteinGym analyses above stop at Stage~1, leaving the full
...
requirement than the joint illustration's original policy---the advantage did not survive
...
\subsubsection{Numerical and Monte Carlo checks}
