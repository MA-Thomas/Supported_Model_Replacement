...
\longrightarrow T\\
\longrightarrow
\text{lexicographic worst-target CNAP regret against }S_0
...
deficit across a declared target-prevalence range.
...
If $T$ contains more than one system, declare a nonempty compact operational
...
singleton, the worst-target CNAP rule is not invoked. Its selected member is
not thereby guaranteed to have small CNAP regret. Worst-target regret resolves
...
Worst-target CNAP reduction & Tied set $T$ & Fixed admissible set $S_0$ over $\Pi_{\mathrm{op}}$ & Operational choice $S_{\mathrm{op}}$ or remaining tie \\
\bottomrule
\end{tabularx}
\end{table}
...
plural:] Over the prespecified operational prevalence set $\Pi_{\mathrm{op}}$,
system [name] had the lexicographically smallest descending range-regret
vector. Its largest regret was [value] in [evaluation], relative to
[admissible comparator] at prevalence [value]. The operational result was
...
or give worst-target regret priority over the incoming-survival comparison.
\end{quote}
