...
\large A Finite-Evidence, Refutation-Based Rule for Average Precision and AUROC}
...
$\psi^{\mathrm{obs}}_{K_E,d}=\binom{R}{K_E}/\binom{M}{K_E}$ is strictly
increasing in $R$, so $\psi^{\mathrm{obs}}_{K_E,d}>\gamma$ holds if and only if
\[
R\geq r^\star,
\qquad
