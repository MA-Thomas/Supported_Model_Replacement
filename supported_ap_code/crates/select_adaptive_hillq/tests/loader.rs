...
fn loads_rosters_and_passes_baseline_audit() {
    ...
    // E1 candidates: -0.5 (HLA A), -1.5 (HLA B), -3.0 (HLA C)
    // -> max -0.5, lse computed below.
    // E2 candidate: -2.0 (singleton) -> max/lse -2.0.
    let lse_e1 = -0.5_f64 + ((0.0_f64).exp() + (-1.0_f64).exp() + (-2.5_f64).exp()).ln();
    ...
    let mapping = "patient_id,mutation,long_peptide,env_id,nmer,HLA-RE,mapping_status\n\
        ...
        P1,M1,LP1,11,DDDDDDDDD,C,scoreable\n\
    ...
    assert_eq!(task.raw_candidates[0], vec![-0.5, -1.5, -3.0]);
    ...
    assert_eq!(audit.n_scoreable_candidates, 4);
    assert_eq!(audit.n_zero_signal_candidates, 0);
...
#[test]
fn rejects_coverage_by_omission_floor_rows() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    let transfer = root.join("pdac/full_hla/pr");
    let mapping_path = root.join("normalized_inputs/mapping.csv");
    ...
    write(
        &transfer.join("long_peptide_predictions.csv"),
        "patient_id,mutation,long_peptide,l2_variant,score,label\n\
         P1,M1,LP1,max,-0.5,1\n\
         P1,M1,LP1,logsumexp,-0.5,1\n",
    );
    write(
        &transfer.join("target_tau_selection_by_observation.csv"),
        "patient_id,env_id,peptide,hla,score,obs_idx\n\
    ...
    write(
        ...
        "patient_id,mutation,long_peptide,env_id,nmer,HLA-RE,mapping_status\n\
         P1,M1,LP1,10,AAAAAAAAA,A,scoreable\n\
         P1,M1,LP1,10,BBBBBBBBB,B,floor\n",
    );
    write(
        &transfer.join("summary.json"),
        &format!("{{\"mapping\": \"{}\"}}", mapping_path.display()),
    ...
    let error = load_task(root, "pdac", "full_hla", "pr").unwrap_err();
    assert!(
        error
            .to_string()
            .contains("complete-F selection requires every mapping row to be scoreable"),
