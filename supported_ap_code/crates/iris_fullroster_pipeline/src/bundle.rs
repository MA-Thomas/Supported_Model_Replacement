...
pub fn build_tournament_bundles(
    config_path: &Path,
    ...
    let result = (|| {
        ...
        for branch in BRANCHES {
            ...
            let report = build_metric_bundle(
                &config,
                transfer_package,
...
fn build_metric_bundle(
    ...
    let registry = tournament_registry(config, branch)?;
    ...
    for cohort_id in COHORTS {
        ...
        let source_records = write_tournament_scores(
    ...
    if registry.systems.len() != 65 {
        bail!(
            "tournament bundle has {} systems instead of 65",
...
fn tournament_registry(config: &PipelineConfig, branch: &str) -> Result<SystemRegistry> {
    let mut systems = Vec::with_capacity(65);
    for model_id in MODELS {
        ...
        let l2_id = config.adaptive_l2.id();
        let system_id = format!("{model_id}__{l2_id}");
        systems.push(SystemRecord {
            display_label: format!("{} / Frozen endpoint-local epitope + second-HLA hybrid", model.display_label),
            score_column: format!("score_{system_id}"),
            system_id,
            annotations: BTreeMap::from([
                ("provider_domain".into(), json!("IRIS")),
                ...
                (
                    ...
                    json!("endpoint_local_epitope_second_hla_hybrid"),
                ),
                (
                    "l2_aggregation_spec".into(),
                    serde_json::to_value(&config.adaptive_l2)?,
                ),
                ("l2_aggregation_formula".into(), json!(config.adaptive_l2.formula())),
                ("development_scope".into(), json!("parameters developed in full_hla; frozen unchanged for transport across all component models")),
                ("nci_summary_sha256".into(), json!(model.nci_summary_sha256)),
            ]),
        });
    }
...
fn tournament_spec(
    ...
    let mut l2_ids: Vec<_> = config.fixed_l2.iter().map(L2Spec::id).collect();
    l2_ids.push(config.adaptive_l2.id().to_owned());
        ...
        annotations: BTreeMap::from([
            ...
            (
                ...
                json!({
                    ...
                    "frozen_adaptive_l2": config.adaptive_l2,
                    "frozen_adaptive_l2_formula": config.adaptive_l2.formula(),
    ...
    for model_id in MODELS {
        ...
        for record in reader.records() {
            ...
            if !observed.insert((system_id.clone(), authority_row.endpoint_id.clone())) {
                bail!("duplicate tournament score");
            }
            scores
                .entry(system_id)
                .or_default()
...
mod tests {
    ...
    #[test]
    fn tournament_spec_exposes_roster_and_evaluation_contracts() {
        let config = template_config();
        let registry = tournament_registry(&config, "pr").unwrap();
        let spec = tournament_spec(&config, "pr", &registry).unwrap();
        let contract = &spec.annotations["scientific_contract"];
        assert_eq!(registry.systems.len(), 65);
        assert_eq!(contract["systems_per_component_model"], 13);
        assert_eq!(
            registry
                .systems
                .iter()
                .filter(|system| system.system_id.ends_with(config.adaptive_l2.id()))
                .count(),
            5
        );
        assert_eq!(
