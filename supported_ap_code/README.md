...
`domain_study/proteingym_nested/` is an isolated conditional pedagogical
illustration of the full `M>1`, `K_E=2`, `K_C=2` calculation. It reuses the
retained ProteinGym publication scores, compares ESM-1v ensemble with ESM-1v
single over a hypothetical 40%--50% target-prevalence range, and resamples
residue-position clusters within assays. Its assumptions and outputs are
explicitly labeled illustrative rather than a ProteinGym replacement claim.
