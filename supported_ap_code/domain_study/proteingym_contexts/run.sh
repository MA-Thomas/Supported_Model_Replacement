...
case "$mode" in
  render) exec "$python_bin" "$study_dir/context_story.py" --profile publication --render-only "$@" ;;
  quick|publication)
    cargo build --manifest-path "$crate_dir/Cargo.toml" --release --example proteingym_contexts --offline
    exec "$python_bin" "$study_dir/context_story.py" --profile "$mode" "$@" ;;
