...
#[test]
fn external_multiplicities_reuse_the_exact_retained_effect_path() {
    let evaluation = nested_examples().remove(0);
    let target = target(&[0.05, 0.2, 0.5]);
    let multiplicities = vec![2, 0, 1, 1, 0, 2];
    let (forward, reverse) = evaluation
        .retained_effects_for_multiplicities(&multiplicities, &target, SearchOptions::default())
        .unwrap();
    assert!(forward.value.is_finite());
    assert!(reverse.value.is_finite());
    assert!(forward.value + reverse.value <= 1e-12);

    let missing_class = vec![1, 1, 1, 0, 0, 0];
    assert!(
        evaluation
            .retained_effects_for_multiplicities(&missing_class, &target, SearchOptions::default(),)
            .is_err()
    );
}
